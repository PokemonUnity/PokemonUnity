...
namespace PokemonUnity.Attack
{
	...
	public enum Effects
	{
		...
		///
		/// Moves that [fail]{mechanic:fail}ed, [miss]{mechanic:miss}ed, had [no effect]{mechanic:no_effect}, or were [block]{mechanic:block}ed are still copied.
		///
		...
		x00A = 10,
		/// <summary>
		/// Raises the user's Attack by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x00B = 11,
		/// <summary>
		/// Raises the user's Defense by one stage.<para></para> Raises the user's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// HARDEN,WITHDRAW
		/// </remarks>
		x00C = 12,
		/// <summary>
		/// Raises the user's Special Attack by one stage.<para></para> Raises the user's <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		x00E = 14,
		/// <summary>
		/// Raises the user's evasion by one stage.<para></para> Raises the user's <see cref="Stats.EVASION"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// DOUBLE-TEAM
		/// </remarks>
		x011 = 17,
		/// <summary>
		/// Never misses.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		/// </summary>
		/// <remarks>
		...
		x012 = 18,
		/// <summary>
		/// Lowers the target's Attack by one stage.<para></para> Lowers the target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// GROWL
		/// </remarks>
		x013 = 19,
		/// <summary>
		/// Lowers the target's Defense by one stage.<para></para> Lowers the target's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// TAIL-WHIP,LEER
		/// </remarks>
		x014 = 20,
		/// <summary>
		/// Lowers the target's Speed by one stage.<para></para> Lowers the target's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x015 = 21,
		/// <summary>
		/// Lowers the target's accuracy by one stage.<para></para> Lowers the target's <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x018 = 24,
		/// <summary>
		/// Lowers the target's evasion by one stage.<para></para> Lowers the target's <see cref="Stats.EVASION"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// SWEET-SCENT
		/// </remarks>
		x019 = 25,
		/// <summary>
		/// Resets all Pokémon's stats, accuracy, and evasion.<para></para> Removes [stat]{mechanic:stat}, <see cref="Stats.ACCURACY"/>, and <see cref="Stats.EVASION"/> modifiers from every Pokémon on the [field]{mechanic:field}.
		///
		...
		x01F = 31,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to make the target flinch.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to make the target <seealso cref="IEffectsBattler.Flinch"/>.
		/// </summary>
		/// <remarks>
		...
		x020 = 32,
		/// <summary>
		/// Heals the user by half its max HP.<para></para> Heals the user for half its max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		...
		x021 = 33,
		/// <summary>
		/// Badly poisons the target, inflicting more damage every turn.<para></para> [Badly poisons]{mechanic:badly_poisons} the target.  Never misses when used by a <seealso cref="Types.POISON"/>-type Pokémon.
		/// </summary>
		/// <remarks>
		/// TOXIC
		/// </remarks>
		...
		x025 = 37,
		/// <summary>
		/// User sleeps for two turns, completely healing itself.<para></para> User falls to <see cref="Attack.MoveMetaAilments.SLEEP"/> and immediately regains all its <see cref="Stats.HP"/>.  If the user has another [major status effect]{mechanic:major_status_effect}, <see cref="Attack.MoveMetaAilments.SLEEP"/> will replace it.  The user will always wake up after two turns, or one turn with <seealso cref="Abilities.EARLY_BIRD"/>.
		///
		...
		x026 = 38,
		/// <summary>
		/// Causes a one-hit KO.<para></para> Inflicts damage equal to the target's max <see cref="Stats.HP"/>.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.  This move's <see cref="Stats.ACCURACY"/> is 30% plus 1% for each level the user is higher than the target.  If the user is a lower level than the target, this move will [fail]{mechanic:fail}.
		///
		...
		x027 = 39,
		/// <summary>
		/// Requires a turn to charge before attacking.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User's [critical hit]{mechanic:critical_hit} rate is one level higher when using this move.  User charges for one turn before attacking.
		///
		/// This move cannot be selected by <seealso cref="Moves.SLEEP_TALK"/>.
		/// </summary>
		/// <remarks>
		...
		x028 = 40,
		/// <summary>
		/// Inflicts damage equal to half the target's HP.<para></para> Inflicts [typeless]{mechanic:typeless} damage equal to half the target's remaining <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		...
		x02B = 43,
		/// <summary>
		/// Has an increased chance for a critical hit.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User's [critical hit]{mechanic:critical_hit} rate is one level higher when using this move.
		/// </summary>
		/// <remarks>
		...
		x02F = 47,
		/// <summary>
		/// Increases the user's chance to score a critical hit.<para></para> User's [critical hit]{mechanic:critical_hit} rate is two levels higher until it leaves the field.  If the user has already used <seealso cref="Moves.FOCUS_ENERGY"/> since entering the field, this move will [fail]{mechanic:fail}.
		///
		...
		x032 = 50,
		/// <summary>
		/// Raises the user's Attack by two stages.<para></para> Raises the user's <see cref="Stats.ATTACK"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// SWORDS-DANCE
		/// </remarks>
		x033 = 51,
		/// <summary>
		/// Raises the user's Defense by two stages.<para></para> Raises the user's <see cref="Stats.DEFENSE"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x034 = 52,
		/// <summary>
		/// Raises the user's Speed by two stages.<para></para> Raises the user's <see cref="Stats.SPEED"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// AGILITY,ROCK-POLISH
		/// </remarks>
		x035 = 53,
		/// <summary>
		/// Raises the user's Special Attack by two stages.<para></para> Raises the user's <see cref="Stats.SPATK"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// NASTY-PLOT
		/// </remarks>
		x036 = 54,
		/// <summary>
		/// Raises the user's Special Defense by two stages.<para></para> Raises the user's <see cref="Stats.SPDEF"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// AMNESIA
		/// </remarks>
		x037 = 55,
		/// <summary>
		/// User becomes a copy of the target until it leaves battle.<para></para> User copies the target's species, weight, type, [ability]{mechanic:ability}, [calculated stats]{mechanic:calculated_stats} (except <see cref="Stats.HP"/>), and moves.  Copied moves will all have 5 [PP]{mechanic:pp} remaining.  <see cref="IBattler.IV"/>s are copied for the purposes of <seealso cref="Moves.HIDDEN_POWER"/>, but stats are not recalculated.
		///
		...
		x03A = 58,
		/// <summary>
		/// Lowers the target's Attack by two stages.<para></para> Lowers the target's <see cref="Stats.ATTACK"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// CHARM,FEATHER-DANCE
		/// </remarks>
		x03B = 59,
		/// <summary>
		/// Lowers the target's Defense by two stages.<para></para> Lowers the target's <see cref="Stats.DEFENSE"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// SCREECH,SHADOW-DOWN
		/// </remarks>
		x03C = 60,
		/// <summary>
		/// Lowers the target's Speed by two stages.<para></para> Lowers the target's <see cref="Stats.SPEED"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x03E = 62,
		/// <summary>
		/// Lowers the target's Special Defense by two stages.<para></para> Lowers the target's <see cref="Stats.SPDEF"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x03F = 63,
		/// <summary>
		/// Reduces damage from physical attacks by half.<para></para> Erects a barrier around the user's side of the field that reduces damage from [physical]{mechanic:physical} attacks by half for five turns.  In double battles, the reduction is 1/3.  [Critical hit]{mechanic:critical_hit}s are not affected by the barrier.
		///
		/// If the user is holding <seealso cref="Items.LIGHT_CLAY"/>, the barrier lasts for eight turns.
		///
		/// <seealso cref="Moves.BRICK_BREAK"/> or <seealso cref="Moves.DEFOG"/> used by an opponent will destroy the barrier.
		...
		x044 = 68,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Attack by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x045 = 69,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Defense by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x046 = 70,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Speed by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x047 = 71,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Special Attack by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x048 = 72,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Special Defense by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x049 = 73,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's accuracy by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x04B = 75,
		/// <summary>
		/// User charges for one turn before attacking.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to make the target flinch.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User charges for one turn before attacking.  [Critical hit chance]{mechanic:critical_hit_chance} is one level higher than normal.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to make the target [flinch]{mechanic:flinch}.
		///
		/// This move cannot be selected by <seealso cref="Moves.SLEEP_TALK"/>.
		/// </summary>
		/// <remarks>
		...
		x04F = 79,
		/// <summary>
		/// Transfers 1/4 of the user's max HP into a doll, protecting the user from further damage or status changes until it breaks.<para></para> Transfers 1/4 the user's max <see cref="Stats.HP"/> into a doll that absorbs damage and causes most negative move effects to [fail]{mechanic:fail}.  If the user leaves the [field]{mechanic:field}, the doll will vanish.  If the user cannot pay the <see cref="Stats.HP"/> cost, this move will fail.
		///
		/// The doll takes damage as normal, using the user's stats and types, and will break when its <see cref="Stats.HP"/> reaches zero.  Self-inflicted damage from <see cref="Attack.MoveMetaAilments.CONFUSION"/> or recoil is not absorbed.  Healing effects from opponents ignore the doll and heal the user as normal.  Moves that work based on the user's <see cref="Stats.HP"/> still do so; the doll's <see cref="Stats.HP"/> does not influence any move.
		///
		/// The doll will block [major status effect]{mechanic:major_status_effect}s, <see cref="Attack.MoveMetaAilments.CONFUSION"/>, and [flinch]{mechanic:flinch}ing.  The effects of <seealso cref="Moves.SMELLING_SALTS"/> and <seealso cref="Moves.WAKE_UP-SLAP"/> do not trigger against a doll, even if the Pokémon behind the doll has the appropriate [major status effect]{mechanic:major_status_effect}.  Multi-turn trapping moves like <seealso cref="Moves.WRAP"/> will hit the doll for their <seealso cref="Attack.MetaCategory.DAMAGE"/>, but the multi-turn trapping and damage effects will not activate.
		///
		...
		///
		/// The doll blocks <seealso cref="Abilities.INTIMIDATE"/>, but all other abilities act as though the doll did not exist.  If the user has an ability that absorbs moves of a certain type for <see cref="Stats.HP"/> (such as <seealso cref="Abilities.VOLT_ABSORB"/> absorbing <seealso cref="Moves.THUNDER_WAVE"/>), such moves will not be blocked.
		///
		/// <seealso cref="Items.LIFE_ORB"/> and berries that cause <see cref="Attack.MoveMetaAilments.CONFUSION"/> still work as normal, but their respective <see cref="Stats.HP"/> loss and <see cref="Attack.MoveMetaAilments.CONFUSION"/> are absorbed/blocked by the doll.
		///
		...
		///
		/// The doll is passed on by <seealso cref="Moves.BATON_PASS"/>.  It keeps its existing <see cref="Stats.HP"/>, but uses the replacement Pokémon's stats and types for damage calculation.
		///
		...
		x051 = 81,
		/// <summary>
		/// If the user is hit after using this move, its Attack rises by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Every time the user is hit after it uses this move but before its next action, its <see cref="Stats.ATTACK"/> raises by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// RAGE
		/// </remarks>
		...
		x054 = 84,
		/// <summary>
		/// Seeds the target, stealing HP from it every turn.<para></para> Plants a seed on the target that [drains]{mechanic:drain} 1/8 of its max <see cref="Stats.HP"/> at the end of every turn and heals the user for the amount taken.  Has no effect on <seealso cref="Types.GRASS"/> Pokémon.  The seed remains until the target leaves the field.
		///
		...
		x05B = 91,
		/// <summary>
		/// Sets the user's and targets's HP to the average of their current HP.<para></para> Changes the user's and target's remaining <see cref="Stats.HP"/> to the average of their current remaining <see cref="Stats.HP"/>.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.  This effect does not count as inflicting damage for other moves and effects that respond to damage taken.
		///
		...
		x05E = 94,
		/// <summary>
		/// Ensures that the user's next move will hit the target.<para></para> If the user targets the same target again before the end of the next turn, the move it uses is guaranteed to hit.  This move itself also ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		///
		...
		x063 = 99,
		/// <summary>
		/// Inflicts more damage when the user has less HP remaining, with a maximum of 200 power.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power varies inversely with the user's proportional remaining <see cref="Stats.HP"/>.
		///
		...
		x065 = 101,
		/// <summary>
		/// Cannot lower the target's HP below 1.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Will not reduce the target's <see cref="Stats.HP"/> below 1.
		/// </summary>
		/// <remarks>
		...
		x066 = 102,
		/// <summary>
		/// Cures the entire party of major status effects.<para></para> Removes [major status effects]{mechanic:major_status_effects} and <see cref="Attack.MoveMetaAilments.CONFUSION"/> from every Pokémon in the user's party.
		/// </summary>
		/// <remarks>
		...
		x06A = 106,
		/// <summary>
		/// Prevents the target from leaving battle.<para></para> The target cannot [switch out]{mechanic:switch_out} normally.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.  This effect ends when the user leaves the [field]{mechanic:field}.
		///
		...
		x06B = 107,
		/// <summary>
		/// Target loses 1/4 its max HP every turn as long as it's asleep.<para></para> Only works on <see cref="Attack.MoveMetaAilments.SLEEP"/>ing Pokémon.  Gives the target a nightmare, damaging it for 1/4 its max <see cref="Stats.HP"/> every turn.  If the target wakes up or leaves the [field]{mechanic:field}, this effect ends.
		/// </summary>
		/// <remarks>
		/// NIGHTMARE
		/// </remarks>
		x06C = 108,
		/// <summary>
		/// Raises the user's evasion by two stages.<para></para> Raises the user's <see cref="Stats.EVASION"/> by two <see cref="IBattler.stages"/>.
		///
		...
		x06D = 109,
		/// <summary>
		/// Ghosts pay half their max HP to hurt the target every turn.  Others decrease Speed but raise Attack and Defense.<para></para> If the user is a <seealso cref="Types.GHOST"/>: user pays half its max <see cref="Stats.HP"/> to place a curse on the target, damaging it for 1/4 its max <see cref="Stats.HP"/> every turn.
		/// Otherwise: Lowers the user's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>, and raises its <see cref="Stats.ATTACK"/> and <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/> each.
		///
		...
		x070 = 112,
		/// <summary>
		/// Scatters Spikes, hurting opposing Pokémon that switch in.<para></para> Scatters spikes around the opposing [field]{mechanic:field}, which damage opposing Pokémon that enter the [field]{mechanic:field} for 1/8 of their max <see cref="Stats.HP"/>.  Pokémon immune to <seealso cref="Types.GROUND"/> moves are immune to this damage, except during <seealso cref="Moves.GRAVITY"/>.  Up to three layers of spikes may be laid down, adding 1/16 to the damage done: two layers of spikes damage for 3/16 max <see cref="Stats.HP"/>, and three layers damage for 1/4 max <see cref="Stats.HP"/>.
		///
		...
		x071 = 113,
		/// <summary>
		/// Forces the target to have no Evade, and allows it to be hit by Normal and Fighting moves even if it's a Ghost.<para></para> Resets the target's <see cref="Stats.EVASION"/> to normal and prevents any further boosting until the target leaves the [field]{mechanic:field}.  A <seealso cref="Types.GHOST"/> under this effect takes normal damage from <seealso cref="Types.NORMAL"/> and <seealso cref="Types.FIGHTING"/> moves.  This move itself ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		/// </summary>
		/// <remarks>
		...
		x073 = 115,
		/// <summary>
		/// Changes the weather to a sandstorm for five turns.<para></para> Changes the weather to a sandstorm for five turns.  Pokémon that are not <seealso cref="Types.GROUND"/>, <seealso cref="Types.ROCK"/>, or <seealso cref="Types.STEEL"/> take 1/16 their max <see cref="Stats.HP"/> at the end of every turn.  Every <seealso cref="Types.ROCK"/> Pokémon's original <see cref="Stats.SPDEF"/> is raised by 50% for the duration of this effect.
		///
		/// <seealso cref="Moves.SOLAR_BEAM"/>'s power is halved.
		///
		/// <seealso cref="Moves.MOONLIGHT"/>, <seealso cref="Moves.MORNING_SUN"/>, and <seealso cref="Moves.SYNTHESIS"/> only heal 1/4 the user's max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		/// SANDSTORM
		/// </remarks>
		x074 = 116,
		/// <summary>
		/// Prevents the user's HP from lowering below 1 this turn.<para></para> The user's <see cref="Stats.HP"/> cannot be lowered below 1 by any means for the remainder of this turn.
		///
		/// If the user successfully used <seealso cref="Moves.DETECT"/>, <seealso cref="Moves.ENDURE"/>, <seealso cref="Moves.PROTECT"/>, <seealso cref="Moves.QUICK_GUARD"/>, or <seealso cref="Moves.WIDE_GUARD"/> on the last turn, this move has a 50% chance to [fail]{mechanic:fail}.
		///
		/// This move cannot be selected by <seealso cref="Moves.ASSIST"/> or <seealso cref="Moves.METRONOME"/>.
		...
		x076 = 118,
		/// <summary>
		/// Raises the target's Attack by two stages and confuses the target.<para></para> Raises the target's <see cref="Stats.ATTACK"/> by two <see cref="IBattler.stages"/>, then [confuses]{mechanic:confuses} it.  If the target's <see cref="Stats.ATTACK"/> cannot be [raised]{mechanic:raised} by two <see cref="IBattler.stages"/>, the <see cref="Attack.MoveMetaAilments.CONFUSION"/> is not applied.
		/// </summary>
		/// <remarks>
		/// SWAGGER
		/// </remarks>
		...
		/// Heals the target for 1/4 its max <see cref="Stats.HP"/>    |    20%
		///
		/// On average, this move inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/> with 52 power and heals the target for 1/20 its max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		/// PRESENT
		/// </remarks>
		...
		x07C = 124,
		/// <summary>
		/// Protects the user's field from major status ailments and confusion for five turns.<para></para> Protects Pokémon on the user's side of the [field]{mechanic:field} from [major status]{mechanic:major_status} effects and <see cref="Attack.MoveMetaAilments.CONFUSION"/> for five turns.  Does not cancel existing ailments.  This effect remains even if the user leaves the [field]{mechanic:field}.
		///
		...
		x07F = 127,
		/// <summary>
		/// Allows the trainer to switch out the user and pass effects along to its replacement.<para></para> User [switches out]{mechanic:switches_out}, and the trainer selects a replacement Pokémon from the party.  [Stat]{mechanic:stat} changes, <see cref="Attack.MoveMetaAilments.CONFUSION"/>, and persistent move effects are passed along to the replacement Pokémon.
		///
		...
		x080 = 128,
		/// <summary>
		/// Has double power against, and can hit, Pokémon attempting to switch out.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the target attempts to [switch out]{mechanic:switch_out} this turn before the user acts, this move hits the target before it leaves and has double power.
		///
		/// This effect can still hit a Pokémon that [switches out]{mechanic:switches_out} when it has a <seealso cref="Moves.SUBSTITUTE"/> up or when an ally has used <seealso cref="Moves.FOLLOW_ME"/>.
		/// </summary>
		/// <remarks>
		/// PURSUIT
		/// </remarks>
		...
		x083 = 131,
		/// <summary>
		/// Heals the user by half its max HP.  Affected by weather.<para></para> Heals the user for half its max <see cref="Stats.HP"/>.
		///
		/// During <seealso cref="Moves.SUNNY_DAY"/>, the healing is increased to 2/3 max <see cref="Stats.HP"/>.
		///
		/// During <seealso cref="Moves.HAIL"/>, <seealso cref="Moves.RAIN_DANCE"/>, or <seealso cref="Moves.SANDSTORM"/>, the healing is decreased to 1/4 max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		...
		x085 = 133,
		/// <summary>
		/// Power and type depend upon user's IVs.  Power can range from 30 to 70.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power and type are determined by the user's <see cref="IBattler.IV"/>s.
		///
		/// Power is given by `x * 40 / 63 + 30`.  `x` is obtained by arranging bit 1 from the <see cref="IBattler.IV"/> for each of <see cref="Stats.SPDEF"/>, <see cref="Stats.SPATK"/>, <see cref="Stats.SPEED"/>, <see cref="Stats.DEFENSE"/>, <see cref="Stats.ATTACK"/>, and <see cref="Stats.HP"/> in that order.  (Bit 1 is 1 if the <see cref="IBattler.IV"/> is of the form `4n + 2` or `4n + 3`.  `x` is then 64 * <see cref="Stats.SPDEF"/> <see cref="IBattler.IV"/> bit 1, plus 32 * <see cref="Stats.SPATK"/> <see cref="IBattler.IV"/> bit 1, etc.)
		///
		/// Power is always between 30 and 70, inclusive.  Average power is 49.5.
		///
		/// Type is given by `y * 15 / 63`, where `y` is similar to `x` above, except constructed from bit 0.  (Bit 0 is 1 if the <see cref="IBattler.IV"/> is odd.)  The result is looked up in the following table.
		///
		/// Value | Type
		/// ----: | --------
		...
		///
		/// This move thus cannot be <seealso cref="Types.NORMAL"/>.  Most other types have an equal 1/16 chance to be selected, given random <see cref="IBattler.IV"/>s.  However, due to the flooring used here, <seealso cref="Types.BUG"/>, <seealso cref="Types.FIGHTING"/>, and <seealso cref="Types.GRASS"/> appear 5/64 of the time, and <seealso cref="Types.DARK"/> only 1/64 of the time.
		/// </summary>
		/// <remarks>
		/// HIDDEN-POWER
		/// </remarks>
		...
		///
		/// <seealso cref="Moves.MOONLIGHT"/>, <seealso cref="Moves.MORNING_SUN"/>, and <seealso cref="Moves.SYNTHESIS"/> heal only 1/4 of the user's max <see cref="Stats.HP"/>.
		///
		/// Pokémon with <seealso cref="Abilities.SWIFT_SWIM"/> have doubled original <see cref="Stats.SPEED"/>.
		///
		/// Pokémon with <seealso cref="Abilities.FORECAST"/> become <seealso cref="Types.WATER"/>.
		///
		/// Pokémon with <seealso cref="Abilities.DRY_SKIN"/> heal 1/8 max <see cref="Stats.HP"/>, Pokémon with <seealso cref="Abilities.HYDRATION"/> are cured of [major status effects]{mechanic:major_status_effects}, and Pokémon with <seealso cref="Abilities.RAIN_DISH"/> heal 1/16 max <see cref="Stats.HP"/> at the end of each turn.
		/// </summary>
		/// <remarks>
		/// RAIN-DANCE
		/// </remarks>
		...
		///
		/// <seealso cref="Moves.MOONLIGHT"/>, <seealso cref="Moves.MORNING_SUN"/>, and <seealso cref="Moves.SYNTHESIS"/> heal 2/3 of the user's max <see cref="Stats.HP"/>.
		///
		/// Pokémon with <seealso cref="Abilities.CHLOROPHYLL"/> have doubled original <see cref="Stats.SPEED"/>.
		///
		/// Pokémon with <seealso cref="Abilities.FORECAST"/> become <seealso cref="Types.FIRE"/>.
		///
		/// Pokémon with <seealso cref="Abilities.LEAF_GUARD"/> are not affected by [major status effects]{mechanic:major_status_effects}.
		///
		/// Pokémon with <seealso cref="Abilities.FLOWER_GIFT"/> change form; every Pokémon on their side of the [field]{mechanic:field} have their original <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> increased by 50%.
		///
		/// Pokémon with <seealso cref="Abilities.DRY_SKIN"/> lose 1/8 max <see cref="Stats.HP"/> at the end of each turn.
		///
		/// Pokémon with <seealso cref="Abilities.SOLAR_POWER"/> have their original <see cref="Stats.SPATK"/> raised by 50% but lose 1/8 their max <see cref="Stats.HP"/> at the end of each turn.
		/// </summary>
		/// <remarks>
		/// SUNNY-DAY
		/// </remarks>
		x08A = 138,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's Defense by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>. Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's <see cref="Stats.DEFENSE"/> one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// STEEL-WING
		/// </remarks>
		x08B = 139,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's Attack by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>. Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's <see cref="Stats.ATTACK"/> one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x08C = 140,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise all of the user's stats by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>. Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise all of the user's stats one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x08D = 141,
		/// <summary>
		/// User pays half its max HP to max out its Attack.<para></para> User pays half its max <see cref="Stats.HP"/> to raise its <see cref="Stats.ATTACK"/> to +6 <see cref="IBattler.stages"/>.  If the user cannot pay the <see cref="Stats.HP"/> cost, this move will [fail]{mechanic:fail}.
		/// </summary>
		/// <remarks>
		/// BELLY-DRUM
		/// </remarks>
		x08F = 143,
		/// <summary>
		/// Discards the user's stat changes and copies the target's.<para></para> Discards the user's [stat changes]{mechanic:stat_changes} and copies the target's.
		///
		/// This move cannot be copied by <seealso cref="Moves.MIRROR_MOVE"/>.
		/// </summary>
		/// <remarks>
		...
		x091 = 145,
		/// <summary>
		/// Raises the user's Defense by one stage.  User charges for one turn before attacking.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Raises the user's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.  User then charges for one turn before attacking.
		///
		/// This move cannot be selected by <seealso cref="Moves.SLEEP_TALK"/>.
		/// </summary>
		/// <remarks>
		...
		x094 = 148,
		/// <summary>
		/// Hits the target two turns later.<para></para> Inflicts [typeless]{mechanic:typeless} <seealso cref="Attack.MetaCategory.DAMAGE"/> at the end of the third turn, starting with this one.  This move cannot score a [critical hit]{mechanic:critical_hit}.  If the target [switches out]{mechanic:switches_out}, its replacement will be hit instead.  Damage is calculated at the time this move is used; [stat changes]{mechanic:stat_changes} and [switching out]{mechanic:switching_out} during the delay won't change the damage inflicted.  No move with this effect can be used against the same target again until after the end of the third turn.
		///
		...
		x09A = 154,
		/// <summary>
		/// Hits once for every conscious Pokémon the trainer has.<para></para> Inflicts [typeless]{mechanic:typeless} <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Every Pokémon in the user's party, excepting those that have fainted or have a [major status effect]{mechanic:major_status_effect}, attacks the target.  Calculated stats are ignored; the base stats for the target and assorted attackers are used instead.  The random factor in the damage formula is not used.  <seealso cref="Types.DARK"/> Pokémon still get [STAB]{mechanic:stab}.
		///
		/// This effect breaks through <seealso cref="Abilities.WONDER_GUARD"/>.
		/// </summary>
		/// <remarks>
		...
		x09C = 156,
		/// <summary>
		/// Raises user's Defense by one stage.<para></para> Raises user's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		///
		...
		x0A0 = 160,
		/// <summary>
		/// Stores energy up to three times for use with Spit Up and Swallow.<para></para> Raises the user's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/> each.  Stores energy for use with <seealso cref="Moves.SPIT_UP"/> and <seealso cref="Moves.SWALLOW"/>.  Up to three levels of energy can be stored, and all are lost if the user leaves the [field]{mechanic:field}.  Energy is still stored even if the stat boosts cannot be applied.
		///
		...
		x0A2 = 162,
		/// <summary>
		/// Recovers 1/4 HP after one Stockpile, 1/2 HP after two Stockpiles, or full HP after three Stockpiles.<para></para> Heals the user depending on the amount of energy stored by <seealso cref="Moves.STOCKPILE"/>: 1/4 its max <see cref="Stats.HP"/> after one use, 1/2 its max <see cref="Stats.HP"/> after two uses, or fully after three uses.  Stored energy is consumed, and the user's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> are reset to what they would be if <seealso cref="Moves.STOCKPILE"/> had not been used.  If the user has no energy stored, this move will [fail]{mechanic:fail}.
		/// </summary>
		/// <remarks>
		/// SWALLOW
		/// </remarks>
		x0A3 = 163,
		/// <summary>
		/// Changes the weather to a hailstorm for five turns.<para></para> Changes the weather to hail for five turns, during which non-<seealso cref="Types.ICE"/> Pokémon are damaged for 1/16 their max <see cref="Stats.HP"/> at the end of every turn.
		///
		...
		x0A6 = 166,
		/// <summary>
		/// Raises the target's Special Attack by one stage and confuses the target.<para></para> Raises the target's <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/>, then [confuses]{mechanic:confuses} it.
		/// </summary>
		/// <remarks>
		/// FLATTER
		/// </remarks>
		...
		x0A8 = 168,
		/// <summary>
		/// Lowers the target's Attack and Special Attack by two stages.  User faints.<para></para> Lowers the target's <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> by two <see cref="IBattler.stages"/>.  User faints.
		/// </summary>
		/// <remarks>
		/// MEMENTO
		/// </remarks>
		...
		///
		/// If the user's ally [switches out]{mechanic:switches_out}, opposing Pokémon may still hit it with <seealso cref="Moves.PURSUIT"/>.
		///
		/// This move cannot be selected by <seealso cref="Moves.ASSIST"/> or <seealso cref="Moves.METRONOME"/>.
		/// </summary>
		/// <remarks>
		...
		x0AE = 174,
		/// <summary>
		/// Raises the user's Special Defense by one stage.  User's Electric moves have doubled power next turn.<para></para> Raises the user's <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/>.  If the user uses an <seealso cref="Types.ELECTRIC"/> move next turn, its power will be doubled.
		/// </summary>
		/// <remarks>
		/// CHARGE
		/// </remarks>
		...
		x0B1 = 177,
		/// <summary>
		/// User and target swap items.<para></para> User and target permanently swap [held item]{mechanic:held_item}s.  Works even if one of the Pokémon isn't holding anything.  If either Pokémon is holding mail, this move will [fail]{mechanic:fail}.
		///
		...
		x0B2 = 178,
		/// <summary>
		/// Copies the target's ability.<para></para> User's ability is replaced with the target's until the user leaves the [field]{mechanic:field}.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		///
		...
		x0B3 = 179,
		/// <summary>
		/// User will recover half its max HP at the end of the next turn.<para></para> At the end of the next turn, user will be healed for half its max <see cref="Stats.HP"/>.  If the user is [switched out]{mechanic:switched_out}, its replacement will be healed instead for half of the user's max HP.  If the user [faint]{mechanic:faint}s or is forcefully switched by <seealso cref="Moves.ROAR"/> or <seealso cref="Moves.WHIRLWIND"/>, this effect will not activate.
		/// </summary>
		/// <remarks>
		/// WISH
		/// </remarks>
		...
		x0B5 = 181,
		/// <summary>
		/// Prevents the user from leaving battle.  User regains 1/16 of its max HP every turn.<para></para> Prevents the user from [switching out]{mechanic:switching_out}.  User regains 1/16 of its max <see cref="Stats.HP"/> at the end of every turn.  If the user was immune to <seealso cref="Types.GROUND"/> attacks, it will now take normal damage from them.
		///
		...
		x0B6 = 182,
		/// <summary>
		/// Lowers the user's Attack and Defense by one stage after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then lowers the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// SUPERPOWER
		/// </remarks>
		x0B7 = 183,
		/// <summary>
		/// Reflects back the first effect move used on the user this turn.<para></para> The first non-damaging move targeting the user this turn that inflicts [major status effect]{mechanic:major_status_effect}s, [stat change]{mechanic:stat_change}s, or [trap]{mechanic:trap}ping effects will be reflected at its user.
		///
		...
		x0BB = 187,
		/// <summary>
		/// Target sleeps at the end of the next turn.<para></para> Puts the target to <see cref="Attack.MoveMetaAilments.SLEEP"/> at the end of the next turn.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.  If the target leaves the [field]{mechanic:field}, this effect is canceled.  If the target has a status effect when this move is used, this move will [fail]{mechanic:fail}.
		///
		...
		x0BC = 188,
		/// <summary>
		/// Target drops its held item.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Target loses its [held item]{mechanic:held_item}.
		///
		/// Neither the user nor the target can recover its item with <seealso cref="Moves.RECYCLE"/>.
		///
		...
		x0BE = 190,
		/// <summary>
		/// Inflicts more damage when the user has more HP remaining, with a maximum of 150 power.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power increases with the user's remaining <see cref="Stats.HP"/> and is given by `150 * HP / max HP`, to a maximum of 150 when the user has full HP.
		/// </summary>
		/// <remarks>
		...
		x0BF = 191,
		/// <summary>
		/// User and target swap abilities.<para></para> User and target switch abilities.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		///
		...
		/// Deep water     | Lowers target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>
		/// Desert         | Lowers target's <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>
		/// Grass          | Puts target to <see cref="Attack.MoveMetaAilments.SLEEP"/>
		/// Mountain       | Makes target [flinch]{mechanic:flinch}
		/// Road           | Lowers target's <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>
		/// Shallow water  | Lowers target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>
		...
		/// Gateway        | Lowers target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>
		...
		/// Sunset         | Lowers target's <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>
		...
		x0C8 = 200,
		/// <summary>
		/// Has an increased chance for a critical hit and a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to <see cref="Attack.MoveMetaAilments.BURN"/> the target.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User's [critical hit]{mechanic:critical_hit} rate is one level higher when using this move. Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to <see cref="Attack.MoveMetaAilments.BURN"/> the target.
		/// </summary>
		/// <remarks>
		/// BLAZE-KICK
		/// </remarks>
		...
		x0CA = 202,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to badly poison the target.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to [badly poison]{mechanic:badly_poison} the target.
		/// </summary>
		/// <remarks>
		/// POISON-FANG
		/// </remarks>
		...
		x0CC = 204,
		/// <summary>
		/// Lowers the user's Special Attack by two stages after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then lowers the user's <see cref="Stats.SPATK"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x0CD = 205,
		/// <summary>
		/// Lowers the target's Attack and Defense by one stage.<para></para> Lowers the target's <see cref="Stats.ATTACK"/> and <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// TICKLE
		/// </remarks>
		x0CE = 206,
		/// <summary>
		/// Raises the user's Defense and Special Defense by one stage.<para></para> Raises the user's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x0D0 = 208,
		/// <summary>
		/// Raises the user's Attack and Defense by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// BULK-UP
		/// </remarks>
		x0D1 = 209,
		/// <summary>
		/// Has an increased chance for a critical hit and a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to poison the target.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User's [critical hit]{mechanic:critical_hit} rate is one level higher when using this move. Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to <see cref="Attack.MoveMetaAilments.POISON"/> the target.
		/// </summary>
		/// <remarks>
		...
		x0D3 = 211,
		/// <summary>
		/// Raises the user's Special Attack and Special Defense by one stage.<para></para> Raises the user's <see cref="Stats.SPATK"/> and <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// CALM-MIND
		/// </remarks>
		x0D4 = 212,
		/// <summary>
		/// Raises the user's Attack and Speed by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// DRAGON-DANCE
		/// </remarks>
		...
		x0D6 = 214,
		/// <summary>
		/// Heals the user by half its max HP.<para></para> Heals the user for half its max <see cref="Stats.HP"/>.  If the user is <seealso cref="Types.FLYING"/>, its <seealso cref="Types.FLYING"/> type is ignored until the end of this turn.
		/// </summary>
		/// <remarks>
		/// ROOST
		/// </remarks>
		x0D7 = 215,
		/// <summary>
		/// Disables moves and immunities that involve flying or levitating for five turns.<para></para> For five turns (including this one), all immunities to <seealso cref="Types.GROUND"/> moves are disabled.  For the duration of this effect, the <see cref="Stats.EVASION"/> of every Pokémon on the field is lowered by two <see cref="IBattler.stages"/>.  Cancels the effects of <seealso cref="Moves.BOUNCE"/>, <seealso cref="Moves.FLY"/>, and <seealso cref="Moves.SKY_DROP"/>.
		///
		...
		x0D8 = 216,
		/// <summary>
		/// Forces the target to have no evasion, and allows it to be hit by Psychic moves even if it's Dark.<para></para> Resets the target's <see cref="Stats.EVASION"/> to normal and prevents any further boosting until the target leaves the [field]{mechanic:field}.  A <seealso cref="Types.DARK"/> Pokémon under this effect takes normal damage from <seealso cref="Types.PSYCHIC"/> moves.  This move itself ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		/// </summary>
		/// <remarks>
		/// MIRACLE-EYE
		/// </remarks>
		...
		x0DA = 218,
		/// <summary>
		/// Lowers user's Speed by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then lowers the user's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x0DB = 219,
		/// <summary>
		/// Power raises when the user has lower Speed, up to a maximum of 150.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power increases with the target's current <see cref="Stats.SPEED"/> compared to the user, given by `1 + 25 * target Speed / user Speed`, capped at 150.
		/// </summary>
		/// <remarks>
		/// GYRO-BALL
		/// </remarks>
		x0DC = 220,
		/// <summary>
		/// User faints.  Its replacement has its HP fully restored and any major status effect removed.<para></para> User faints.  Its replacement's <see cref="Stats.HP"/> is fully restored, and any [major status effect]{mechanic:major_status_effect} is removed.  If the replacement Pokémon is immediately fainted by a switch-in effect, the next replacement is healed by this move instead.
		/// </summary>
		/// <remarks>
		/// HEALING-WISH
		/// </remarks>
		x0DD = 221,
		/// <summary>
		/// Has double power against Pokémon that have less than half their max HP remaining.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the target has less than half its max <see cref="Stats.HP"/> remaining, this move has double power.
		/// </summary>
		/// <remarks>
		/// BRINE
		/// </remarks>
		...
		///
		/// If this move is [super effective]{mechanic:super_effective} and the target is holding a berry that can reduce this move's damage, it will do so, and will not be stolen.
		/// </summary>
		/// <remarks>
		/// PLUCK,BUG-BITE
		/// </remarks>
		x0E1 = 225,
		/// <summary>
		/// For three turns, friendly Pokémon have doubled Speed.<para></para> For the next three turns, all Pokémon on the user's side of the [field]{mechanic:field} have their original <see cref="Stats.SPEED"/> doubled.  This effect remains if the user leaves the [field]{mechanic:field}.
		/// </summary>
		/// <remarks>
		/// TAILWIND
		/// </remarks>
		x0E2 = 226,
		/// <summary>
		/// Raises one of a friendly Pokémon's stats at random by two stages.<para></para> Raises one of the target's stats by two <see cref="IBattler.stages"/>.  The raised stat is chosen at random from any stats that can be raised by two stages.  If no stat is eligible, this move will [fail]{mechanic:fail}.
		///
		...
		x0E4 = 228,
		/// <summary>
		/// User must switch out after attacking.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then the user immediately [switches out]{mechanic:switches_out}, and the trainer selects a replacement Pokémon from the party.  If the target [faint]{mechanic:faint}s from this attack, the user's trainer selects the new Pokémon to send out first.  If the user is the last Pokémon in its party that can battle, it will not [switch out]{mechanic:switch_out}.
		///
		/// The user may be hit by <seealso cref="Moves.PURSUIT"/> when it [switches out]{mechanic:switches_out}, if it has been targeted and <seealso cref="Moves.PURSUIT"/> has not yet been used.
		///
		...
		x0E5 = 229,
		/// <summary>
		/// Lowers the user's Defense and Special Defense by one stage after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then lowers the user's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		...
		x0E6 = 230,
		/// <summary>
		/// Power is doubled if the target has already moved this turn.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the target uses a move or [switches out]{mechanic:switches_out} this turn before this move is used, this move has double power.
		/// </summary>
		/// <remarks>
		/// PAYBACK
		/// </remarks>
		...
		x0E9 = 233,
		/// <summary>
		/// Throws held item at the target; power depends on the item.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power and type are determined by the user's [held item]{mechanic:held_item}.  The item is consumed.  If the user is not holding an item, or its item has no set type and power, this move will [fail]{mechanic:fail}.
		///
		...
		x0EA = 234,
		/// <summary>
		/// Transfers the user's major status effect to the target.<para></para> If the user has a [major status effect]{mechanic:major_status_effect} and the target does not, the user's status is transferred to the target.
		/// </summary>
		/// <remarks>
		/// PSYCHO-SHIFT
		/// </remarks>
		...
		x0EC = 236,
		/// <summary>
		/// Prevents target from restoring its HP for five turns.<para></para> For the next five turns, the target may not use any moves that only restore <see cref="Stats.HP"/>, and move effects that heal the target are disabled.  Moves that steal <see cref="Stats.HP"/> may still be used, but will only inflict damage and not heal the target.
		/// </summary>
		/// <remarks>
		/// HEAL-BLOCK
		/// </remarks>
		x0ED = 237,
		/// <summary>
		/// Power increases against targets with more HP remaining, up to a maximum of 121 power.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power directly relates to the target's relative remaining <see cref="Stats.HP"/>, given by `1 + 120 * current HP / max HP`, to a maximum of 121.
		/// </summary>
		/// <remarks>
		...
		x0EE = 238,
		/// <summary>
		/// User swaps Attack and Defense.<para></para> The user's original <see cref="Stats.ATTACK"/> and <see cref="Stats.DEFENSE"/> are swapped.
		///
		/// This effect is passed on by <seealso cref="Moves.BATON_PASS"/>.
		/// </summary>
		/// <remarks>
		...
		x0F0 = 240,
		/// <summary>
		/// Prevents the target from scoring critical hits for five turns.<para></para> For five turns, opposing Pokémon cannot score [critical hits]{mechanic:critical_hit}.
		/// </summary>
		/// <remarks>
		/// LUCKY-CHANT
		/// </remarks>
		...
		x0F3 = 243,
		/// <summary>
		/// User swaps Attack and Special Attack changes with the target.<para></para> User swaps its <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> [stat modifiers]{mechanic:stat_modifiers} modifiers with the target.
		/// </summary>
		/// <remarks>
		/// POWER-SWAP
		/// </remarks>
		x0F4 = 244,
		/// <summary>
		/// User swaps Defense and Special Defense changes with the target.<para></para> User swaps its <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> modifiers with the target.
		/// </summary>
		/// <remarks>
		/// GUARD-SWAP
		/// </remarks>
		x0F5 = 245,
		/// <summary>
		/// Power increases against targets with more raised stats, up to a maximum of 200.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power starts at 60 and is increased by 20 for every <see cref="IBattler.stages"/> any of the target's stats has been raised, capping at 200.  <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers do not increase this move's power.
		/// </summary>
		/// <remarks>
		/// PUNISHMENT
		/// </remarks>
		...
		x0F9 = 249,
		/// <summary>
		/// Scatters poisoned spikes, poisoning opposing Pokémon that switch in.<para></para> Scatters poisoned spikes around the opposing [field]{mechanic:field}, which <see cref="Attack.MoveMetaAilments.POISON"/> opposing Pokémon that enter the [field]{mechanic:field}.  A second layer of these spikes may be laid down, in which case Pokémon will be [badly poison]{mechanic:badly_poison}ed instead.  Pokémon immune to either <seealso cref="Types.GROUND"/> moves or being <see cref="Attack.MoveMetaAilments.POISON"/>ed are immune to this effect.  Pokémon otherwise immune to <seealso cref="Types.GROUND"/> moves are affected during <seealso cref="Moves.GRAVITY"/>.
		///
		...
		x0FA = 250,
		/// <summary>
		/// User and target swap stat changes.<para></para> User swaps its [stat modifiers]{mechanic:stat_modifiers} with the target.
		/// </summary>
		/// <remarks>
		/// HEART-SWAP
		/// </remarks>
		x0FB = 251,
		/// <summary>
		/// Restores 1/16 of the user's max HP each turn.<para></para> Restores 1/16 of the user's max <see cref="Stats.HP"/> at the end of each turn.  If the user leaves the [field]{mechanic:field}, this effect ends.
		///
		/// This effect is passed on by <seealso cref="Moves.BATON_PASS"/>.
		/// </summary>
		/// <remarks>
		...
		x0FE = 254,
		/// <summary>
		/// User takes 1/4 its max HP in recoil.<para></para> Inflicts [typeless]{mechanic:typeless} <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User takes 1/4 its max <see cref="Stats.HP"/> in recoil.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/> modifiers.
		///
		...
		x108 = 264,
		/// <summary>
		/// Lowers the target's Special Attack by two stages if it's the opposite gender.<para></para> Lowers the target's <see cref="Stats.SPATK"/> by two <see cref="IBattler.stages"/>.  If the user and target are the same gender, or either is genderless, this move will [fail]{mechanic:fail}.
		/// </summary>
		/// <remarks>
		/// CAPTIVATE
		/// </remarks>
		x10A = 266,
		/// <summary>
		/// Causes damage when opposing Pokémon switch in.<para></para> Spreads sharp rocks around the opposing [field]{mechanic:field}, damaging any Pokémon that enters the [field]{mechanic:field} for 1/8 its max <see cref="Stats.HP"/>.  This damage is affected by the entering Pokémon's susceptibility to <seealso cref="Types.ROCK"/> moves.
		///
		...
		x10E = 270,
		/// <summary>
		/// User faints, and its replacement is fully healed.<para></para> User [faint]{mechanic:faint}s.  Its replacement's <see cref="Stats.HP"/> and [PP]{mechanic:pp} are fully restored, and any [major status effect]{mechanic:major_status_effect} is removed.
		/// </summary>
		/// <remarks>
		/// LUNAR-DANCE
		/// </remarks>
		x10F = 271,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's Special Defense by two stages.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to lower the target's <see cref="Stats.SPDEF"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// SEED-FLARE
		/// </remarks>
		...
		x114 = 276,
		/// <summary>
		/// Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's Special Attack by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Has a <see cref="Attack.Data.MetaData.AilmentChance"/> chance to raise the user's <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		...
		x115 = 277,
		/// <summary>
		/// Raises the user's Attack and accuracy by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// HONE-CLAWS
		/// </remarks>
		...
		x117 = 279,
		/// <summary>
		/// Averages Defense and Special Defense with the target.<para></para> Averages the user's unmodified <see cref="Stats.DEFENSE"/> with the target's unmodified Defense; the value becomes the unmodified Defense for both Pokémon. Unmodified <see cref="Stats.SPDEF"/> is averaged the same way.
		/// </summary>
		/// <remarks>
		/// GUARD-SPLIT
		/// </remarks>
		x118 = 280,
		/// <summary>
		/// Averages Attack and Special Attack with the target.<para></para> Averages the user's unmodified <see cref="Stats.ATTACK"/> with the target's unmodified Attack; the value becomes the unmodified Attack for both Pokémon. Unmodified <see cref="Stats.SPATK"/> is averaged the same way.
		///
		...
		x119 = 281,
		/// <summary>
		/// All Pokémon's Defense and Special Defense are swapped for 5 turns.<para></para> For five turns (including this one), every Pokémon's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> are swapped.
		/// </summary>
		/// <remarks>
		/// WONDER-ROOM
		/// </remarks>
		x11A = 282,
		/// <summary>
		/// Inflicts damage based on the target's Defense, not Special Defense.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Damage calculation always uses the target's <see cref="Stats.DEFENSE"/>, regardless of this move's damage class.
		/// </summary>
		/// <remarks>
		...
		x11C = 284,
		/// <summary>
		/// Raises the user's Speed by two stages and halves the user's weight.<para></para> Raises the user's <see cref="Stats.SPEED"/> by two <see cref="IBattler.stages"/>.  Halves the user's weight; this effect does not stack.
		/// </summary>
		/// <remarks>
		/// AUTOTOMIZE
		/// </remarks>
		x11D = 285,
		/// <summary>
		/// Moves have 100% accuracy against the target for three turns.<para></para> For three turns (including this one), moves used against the target have 100% <see cref="Stats.ACCURACY"/>, but the target is immune to <seealso cref="Types.GROUND"/> damage.  Accuracy of one-hit KO moves is exempt from this effect.
		///
		...
		x120 = 288,
		/// <summary>
		/// Always scores a critical hit.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Always scores a [critical hit]{mechanic:critical_hit}.
		/// </summary>
		/// <remarks>
		...
		x121 = 289,
		/// <summary>
		/// Deals splash damage to Pokémon next to the target.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If this move successfully hits the target, any Pokémon adjacent to the target are damaged for 1/16 their max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		/// FLAME-BURST
		/// </remarks>
		x122 = 290,
		/// <summary>
		/// Raises the user's Special Attack, Special Defense, and Speed by one stage each.<para></para> Raises the user's <see cref="Stats.SPATK"/>, <see cref="Stats.SPDEF"/>, and <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// QUIVER-DANCE
		/// </remarks>
		...
		x125 = 293,
		/// <summary>
		/// Power is higher when the user has greater Speed than the target, up to a maximum of 150.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  The greater the user's <see cref="Stats.SPEED"/> compared to the target's, the higher power this move has, to a maximum of 150.
		///
		/// User's Speed                     | Power
		/// -------------------------------- | ----:
		...
		x127 = 295,
		/// <summary>
		/// Inflicts regular damage.  Raises the user's Speed by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Raises the user's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// FLAME-CHARGE
		/// </remarks>
		x128 = 296,
		/// <summary>
		/// Lowers the target's Special Defense by two stages.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the target's <see cref="Stats.SPDEF"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// ACID-SPRAY
		/// </remarks>
		...
		x12C = 300,
		/// <summary>
		/// Makes the target act next this turn.<para></para> The target will act next this turn, regardless of <see cref="Stats.SPEED"/> or move priority.
		...
		x12F = 303,
		/// <summary>
		/// Ignores the target's stat modifiers.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Damage calculation ignores the target's [stat modifiers]{mechanic:stat_modifiers}, including <see cref="Stats.EVASION"/>.
		/// </summary>
		/// <remarks>
		...
		x130 = 304,
		/// <summary>
		/// Removes all of the target's stat modifiers.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  All of the target's [stat modifiers]{mechanic:stat_modifiers} are reset to zero.
		/// </summary>
		/// <remarks>
		/// CLEAR-SMOG
		/// </remarks>
		x131 = 305,
		/// <summary>
		/// Power is higher the more the user's stats have been raised, to a maximum of 31×.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Power is increased by 100% its original value for every <see cref="IBattler.stages"/> any of the user's stats have been raised.  <see cref="Stats.ACCURACY"/>, <see cref="Stats.EVASION"/>, and lowered stats do not affect this move's power.  For a Pokémon with all five stats modified to +6, this move's power is 31×.
		/// </summary>
		/// <remarks>
		...
		x134 = 308,
		/// <summary>
		/// Raises user's Attack, Special Attack, and Speed by two stages.  Lower user's Defense and Special Defense by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/>, <see cref="Stats.SPATK"/>, and <see cref="Stats.SPEED"/> by two <see cref="IBattler.stages"/> each.  Lowers the user's <see cref="Stats.DEFENSE"/> and <see cref="Stats.SPDEF"/> by one <seealso cref="Mechanics.STAGE"/> each.
		/// </summary>
		/// <remarks>
		/// SHELL-SMASH
		/// </remarks>
		x135 = 309,
		/// <summary>
		/// Heals the target for half its max HP.<para></para> Heals the target for half its max <see cref="Stats.HP"/>.
		/// </summary>
		/// <remarks>
		/// HEAL-PULSE
		/// </remarks>
		x136 = 310,
		/// <summary>
		/// Has double power if the target has a major status ailment.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the target has a [major status ailment]{mechanic:major_status_ailment}, this move has double power.
		/// </summary>
		/// <remarks>
		/// HEX
		/// </remarks>
		...
		x138 = 312,
		/// <summary>
		/// Raises the user's Attack by one stage and its Speed by two stages.<para></para> Raises the user's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/> and its <see cref="Stats.SPEED"/> by two stages.
		/// </summary>
		/// <remarks>
		/// SHIFT-GEAR
		/// </remarks>
		x139 = 313,
		/// <summary>
		/// Ends wild battles.  Forces trainers to switch Pokémon.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>, then [switches]{mechanic:switch_out} the target out for another of its trainer's Pokémon, selected at random.
		///
		...
		x13A = 314,
		/// <summary>
		/// Destroys the target's held berry.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the target is [holding]{mechanic:held_item} a [berry]{mechanic:berry}, it's destroyed and cannot be used in response to this move.
		/// </summary>
		/// <remarks>
		/// INCINERATE
		/// </remarks>
		x13B = 315,
		/// <summary>
		/// Makes the target act last this turn.<para></para> Forces the target to act last this turn, regardless of <see cref="Stats.SPEED"/> or move [priority]{mechanic:priority}.  If the target has already acted this turn, this move will [fail]{mechanic:fail}.
		/// </summary>
		/// <remarks>
		/// QUASH
		/// </remarks>
		x13C = 316,
		/// <summary>
		/// Raises the user's Attack and Special Attack by one stage.<para></para> Raises the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/> each.  During <seealso cref="Moves.SUNNY_DAY"/>, raises both stats by two stages.
		/// </summary>
		/// <remarks>
		/// GROWTH
		/// </remarks>
		x13D = 317,
		/// <summary>
		/// Has double power if the user has no held item.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If the user has no [held item]{mechanic:held_item}, this move has double power.
		/// </summary>
		/// <remarks>
		/// ACROBATICS
		/// </remarks>
		...
		x140 = 320,
		/// <summary>
		/// Inflicts damage equal to the user's remaining HP.  User faints.<para></para> Inflicts damage equal to the user's remaining <see cref="Stats.HP"/>.  User faints.
		/// </summary>
		/// <remarks>
		/// FINAL-GAMBIT
		/// </remarks>
		x141 = 321,
		/// <summary>
		/// Raises the user's Special Attack by three stages.<para></para> Raises the user's <see cref="Stats.SPATK"/> by three <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// TAIL-GLOW
		/// </remarks>
		x142 = 322,
		/// <summary>
		/// Raises the user's Attack, Defense, and accuracy by one stage each.<para></para> Raises the user's <see cref="Stats.ATTACK"/>, <see cref="Stats.DEFENSE"/>, and <see cref="Stats.ACCURACY"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// COIL
		/// </remarks>
		x143 = 323,
		/// <summary>
		/// Gives the user's held item to the target.<para></para> Transfers the user's [held item]{mechanic:held_item} to the target.  If the user has no held item, or the target already has a held item, this move will [fail]{mechanic:fail}.
		/// </summary>
		/// <remarks>
		/// BESTOW
		/// </remarks>
		...
		x146 = 326,
		/// <summary>
		/// With <seealso cref="Moves.FIRE_PLEDGE"/>, damages opposing Pokémon for 1/8 their max HP every turn for four turns.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  If a friendly Pokémon used <seealso cref="Moves.FIRE_PLEDGE"/> earlier this turn, all opposing Pokémon will take 1/8 their max <see cref="Stats.HP"/> in damage at the end of every turn for four turns (including this one).
		/// </summary>
		/// <remarks>
		/// GRASS-PLEDGE
		/// </remarks>
		x147 = 327,
		/// <summary>
		/// Raises the user's Attack and Special Attack by one stage each.<para></para> Raises the user's <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// WORK-UP
		/// </remarks>
		x148 = 328,
		/// <summary>
		/// Raises the user's Defense by three stages.<para></para> Raises the user's <see cref="Stats.DEFENSE"/> by three <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// COTTON-GUARD
		/// </remarks>
		...
		x14A = 330,
		/// <summary>
		/// Lowers the target's Speed by one stage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the target's <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// GLACIATE
		/// </remarks>
		...
		x14E = 334,
		/// <summary>
		/// Lowers the user's Defense, Special Defense, and Speed by one stage each.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the user's <see cref="Stats.DEFENSE"/>, <see cref="Stats.SPDEF"/>, and <see cref="Stats.SPEED"/> by one <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// V-CREATE
		/// </remarks>
		...
		x17C = 380,
		/// <summary>
		/// Never misses.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Ignores <see cref="Stats.ACCURACY"/> and <see cref="Stats.EVASION"/>.
		/// </summary>
		/// <remarks>
		/// DISARMING-VOICE
		/// </remarks>
		...
		x186 = 390,
		/// <summary>
		/// Guarantees a critical hit with the user's next move.<para></para> The user's next move will result in a [critical hit]{mechanic:critical_hit}.
		/// </summary>
		/// <remarks>
		/// LASER-FOCUS
		/// </remarks>
		...
		x18B = 395,
		/// <summary>
		/// Lowers the target's Attack by one stage after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the target's <see cref="Stats.ATTACK"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// LUNGE,TROP-KICK
		/// </remarks>
		x18C = 396,
		/// <summary>
		/// Lowers the target's Defense by one stage after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the target's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// FIRE-LASH
		/// </remarks>
		...
		x18E = 398,
		/// <summary>
		/// Exchanges the user's Speed with the target's.<para></para> Exchanges the original <see cref="Stats.SPEED"/> stats of the user and target.
		/// </summary>
		/// <remarks>
		/// SPEED-SWAP
		/// </remarks>
		...
		x194 = 404,
		/// <summary>
		/// Lowers the user's Defense by one stage after inflicting damage.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  Lowers the user's <see cref="Stats.DEFENSE"/> by one <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// CLANGING-SCALES
		/// </remarks>
		...
		x19B = 411,
		/// <summary>
		/// Lowers the target's Attack and Special Attack by one stage.<para></para> Lowers the target's <see cref="Stats.ATTACK"/> and <see cref="Stats.SPATK"/> by one stage each.
		/// </summary>
		/// <remarks>
		/// TEARFUL-LOOK
		/// </remarks>
		...
		x19D = 413,
		/// <summary>
		/// Raises all of the user's stats by two stages.<para></para> Raises the user's <see cref="Stats.ATTACK"/>, <see cref="Stats.DEFENSE"/>, <see cref="Stats.SPATK"/>, <see cref="Stats.SPDEF"/>, and <see cref="Stats.SPEED"/> by two <see cref="IBattler.stages"/> each.
		/// </summary>
		/// <remarks>
		/// EXTREME-EVOBOOST
		/// </remarks>
		...
		x1A4 = 420,
		/// <summary>
		/// Has an increased chance for a critical hit in Hyper Mode.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User's [critical hit]{mechanic:critical_hit} rate is one level higher when using this move while in [hyper mode]{mechanic:hyper_mode}.
		/// </summary>
		/// <remarks>
		/// SHADOW-RUSH
		/// </remarks>
		x711 = 10001,
		/// <summary>
		/// User receives 1/2 its HP in recoil.<para></para> Inflicts <seealso cref="Attack.MetaCategory.DAMAGE"/>.  User takes 1/2 of its current <see cref="Stats.HP"/> in recoil.
		/// </summary>
		/// <remarks>
		/// SHADOW-END
		/// </remarks>
		x712 = 10002,
		/// <summary>
		/// Halves HP of all Pokémon on the field.  Must recharge<para></para> Halves <see cref="Stats.HP"/> of all Pokémon on the field.  User loses its next turn to "recharge", and cannot attack or [switch]{mechanic:switch} out during that turn.
		/// </summary>
		/// <remarks>
		/// SHADOW-HALF
		/// </remarks>
		x713 = 10003,
		/// <summary>
		/// Lowers the target's evasion by two stages.<para></para> Lowers the target's <see cref="Stats.EVASION"/> by two <see cref="IBattler.stages"/>.
		/// </summary>
		/// <remarks>
		/// SHADOW-MIST
		/// </remarks>
		...
		x715 = 10005,
		/// <summary>
		/// Changes the weather to Shadow Sky for five turns.<para></para> Changes the weather to Shadow Sky for five turns.  Pokémon other than []{type:shadow} Pokémon take 1/16 their max <see cref="Stats.HP"/> at the end of every turn.  This move is typeless for the purposes of <seealso cref="Moves.WEATHER_BALL"/>.
		/// </summary>
		/// <remarks>
		/// SHADOW-SKY
		/// </remarks>
