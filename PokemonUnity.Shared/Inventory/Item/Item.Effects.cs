...
namespace PokemonUnity.Inventory
{
    ...
    {
public static ItemUseResults pbRepel(Items item,int steps) {
  if (Game.GameData.RepelSteps>0) {
    Game.pbMessage(Game._INTL("But the effects of a Repel lingered from earlier."));
    //return 0;
...
/* ToDo: Uncomment after setting up UI interface classes
...
UseFromBag.Add(Items.BLACK_FLUTE, () => {
   Game.pbMessage(Game._INTL("{1} used the {2}.",Game.GameData.Player.Name,Items.BLACK_FLUTE.ToString(TextScripts.Name)));
   Game.pbMessage(Game._INTL("Wild Pokémon will be repelled."));
   //Game.GameData.MapData.blackFluteUsed=true;
   //Game.GameData.MapData.whiteFluteUsed=false;
   //next 1;
   return ItemUseResults.UsedNotConsumed;
});
UseFromBag.Add(Items.WHITE_FLUTE, () => {
   Game.pbMessage(Game._INTL("{1} used the {2}.",Game.GameData.Player.Name,Items.WHITE_FLUTE.ToString(TextScripts.Name)));
   Game.pbMessage(Game._INTL("Wild Pokémon will be lured."));
   //Game.GameData.MapData.blackFluteUsed=false;
   //Game.GameData.MapData.whiteFluteUsed=true;
   //next 1;
   return ItemUseResults.UsedNotConsumed;
});
...
UseFromBag.Add(Items.ESCAPE_ROPE, () => {
   ...
   //  return ItemUseResults.NotUsed;
   //}
   //if ((Game.GameData.Global.escapePoint != null) && Game.GameData.Global.escapePoint.Length>0) {
...
UseFromBag.Add(Items.SACRED_ASH, () => {
   ...
   //   IPokemonScreen screen=new PokemonScreen(scene,Game.GameData.Player.Party);
   //   screen.pbStartScene(Game._INTL("Using item..."),false);
   //   foreach (Pokemon i in Game.GameData.Player.Party) {
   //    if (i.HP<=0 && !i.isEgg) {
   //      revived+=1;
   //      i.Heal();
...
UseFromBag.Add(Items.BICYCLE, () => {
   ...
   //return Game.pbBikeCheck() ? ItemUseResults.CloseBagNotConsumed : ItemUseResults.NotUsed;
   return ItemUseResults.NotUsed;
});
...
UseFromBag.Add(Items.OLD_ROD, () => {
   //Terrains terrain=Game.pbFacingTerrainTag();
   //bool notCliff=Game.GameData.GameMap.passable(Game.GameData.GamePlayer.x,Game.GameData.GamePlayer.y,Game.GameData.GamePlayer.direction);
   //if ((Terrain.isWater(terrain) && !Game.GameData.Global.surfing && notCliff) ||
   //   (Terrain.isWater(terrain) && Game.GameData.Global.surfing)) {
   //  //next 2;
...
UseFromBag.Add(Items.TOWN_MAP, () => {
   //Game.UI.pbShowMap(-1,false);
   //next 1; // Continue
   return ItemUseResults.UsedNotConsumed; 
});
UseFromBag.Add(Items.COIN_CASE, () => {
   Game.pbMessage(Game._INTL("Coins: {1}",Game.GameData.Player.Coins));
   //next 1; // Continue
   return ItemUseResults.UsedNotConsumed; 
});
UseFromBag.Add(Items.EXP_ALL, () => {
   //Game.GameData.Bag.pbChangeItem(Items.EXP_ALL, Items.EXP_ALL_OFF);
   Game.pbMessage(Game._INTL("The Exp Share was turned off."));
   //next 1; // Continue
   return ItemUseResults.UsedNotConsumed; 
});
UseFromBag.Add(Items.EXP_ALL_OFF, () => {
   //Game.GameData.Bag.pbChangeItem(Items.EXP_ALL_OFF, Items.EXP_ALL);
   Game.pbMessage(Game._INTL("The Exp Share was turned on."));
   //next 1; // Continue
   return ItemUseResults.UsedNotConsumed; 
});
#endregion
...
UseInField.Add(Items.HONEY, () => {  
   Game.pbMessage(Game._INTL("{1} used the {2}.",Game.GameData.Player.Name,Items.HONEY.ToString(TextScripts.Name)));
   //Game.pbSweetScent();
});
UseInField.Add(Items.ESCAPE_ROPE, () => {
   //int[] escape=(Game.GameData.Global.escapePoint); //rescue null
   ...
   //   Game.GameData.GameTemp.player_new_map_id=escape[0];
   //   Game.GameData.GameTemp.player_new_x=escape[1];
   //   Game.GameData.GameTemp.player_new_y=escape[2];
   //   Game.GameData.GameTemp.player_new_direction=escape[3];
...
UseInField.Add(Items.OLD_ROD, () => {
   //Terrains terrain=Game.pbFacingTerrainTag();
   //bool notCliff=Game.GameData.GameMap.passable/(Game.GameData.GamePlayer.x,Game.GameData.GamePlayer.y,Game.GameData.GamePlayer.direction);
   //if (!Terrain.isWater(terrain) || (!notCliff && !Game.GameData.Global.surfing)) {
     Game.pbMessage(Game._INTL("Can't use that here."));
     //next;
     return;
   //}
   //bool encounter=Game.GameData.PokemonEncounters.hasEncounter(EncounterTypes.OldRod);
   //if (Game.GameData.pbFishing(encounter,1)) {
   //  Game.GameData.pbEncounter(EncounterTypes.OldRod);
   //}
});
...
UseInField.Add(Items.GOOD_ROD, () => {
   ...
   //if (!Terrain.isWater(terrain) || (!notCliff && !Game.GameData.Global.surfing)) {
     ...
     return;
   //}
   //bool encounter=Game.GameData.PokemonEncounters.hasEncounter(EncounterTypes.GoodRod);
   //if (Game.GameData.pbFishing(encounter,2)) {
   //  Game.GameData.pbEncounter(EncounterTypes.GoodRod);
   //}
});
...
UseInField.Add(Items.SUPER_ROD, () => {
   ...
   //if (!Terrain.isWater(terrain) || (!notCliff && !Game.GameData.Global.surfing)) {
     ...
     return;
   //}
   //bool encounter=Game.GameData.PokemonEncounters.hasEncounter(EncounterTypes.SuperRod);
   //if (Game.GameData.pbFishing(encounter,3)) {
   //  Game.GameData.pbEncounter(EncounterTypes.SuperRod);
   //}
});
...
UseInField.Add(Items.DOWSING_MACHINE, () => {//item == Items.ITEM_FINDER || item == Items.DOWSING_MCHN || 
   ...
   //if (@event == null) {
     Game.pbMessage(Game._INTL("... ... ... ...Nope!\r\nThere's no response."));
   //} else {
   //  int offsetX=@event.x-Game.GameData.GamePlayer.x;
   //  int offsetY=@event.y-Game.GameData.GamePlayer.y;
   //  if (offsetX==0 && offsetY==0) {
   //    for (int i = 0; i < 32; i++) {
   //      Graphics.update();
   //      Input.update();
   //      if ((i&7)==0) Game.GameData.GamePlayer.turn_right_90();
   //      Game.pbUpdateSceneMap();
   //    }
   //    Game.pbMessage(Game._INTL(@"The {1}'s indicating something right underfoot!\1",item.ToString(TextScripts.Name)));
   //  } else {
   //    int direction=Game.GameData.GamePlayer.direction;
   ...
   //      Input.update();
   //      if (i==0) {
   //        if (direction==2) Game.GameData.GamePlayer.turn_down();
   //        if (direction==4) Game.GameData.GamePlayer.turn_left();
   //        if (direction==6) Game.GameData.GamePlayer.turn_right();
   //        if (direction==8) Game.GameData.GamePlayer.turn_up();
   //      }
   //      Game.pbUpdateSceneMap();
   //    }
   //    Game.pbMessage(Game._INTL(@"Huh?\nThe {1}'s responding!\1",item.ToString(TextScripts.Name)));
   //    Game.pbMessage(Game._INTL("There's an item buried around here!"));
   //  }
   //}
});

UseInField.Add(Items.TOWN_MAP, () => {
   //Game.pbShowMap(-1,false);
});
   ...
   if (newspecies<=0) {
     ...
     Game.UI.pbFadeOutInWithMusic(99999, () => {
        ...
        if (scene is IPokemonBag_Scene) {
          ...
          scene.pbRefreshAnnotations(() => { Evolution.pbCheckEvolution(pokemon, item)[0] > 0; });
          scene.pbRefresh();
        }
     });
     //next true;
...
UseOnPokemon.Add(Items.GRACIDEA, (item, pokemon, scene) => {
   if (pokemon.Species == Pokemons.SHAYMIN && pokemon.FormId==0 &&
      pokemon.Status!=Status.FROZEN && !Game.IsNight) {
     if (pokemon.HP>0) {
       pokemon.SetForm(1);
       scene.pbRefresh();
   ...
   if (pokemon.Species == Pokemons.KYUREM) {
     if (pokemon.HP>0) {
       if (pokemon.fused!=null) {
         if (Game.GameData.Player.Party.Length>=6) { //ToDo: Party count has 2 slots open
           scene.pbDisplay(Game._INTL("You have no room to separate the Pokémon."));
           return false;
         } else {
           Game.GameData.Player.Party[Game.GameData.Player.Party.Length]=pokemon.fused[1];
           pokemon.fused=null;
           pokemon.SetForm(0);
           scene.pbHardRefresh();
           scene.pbDisplay(Game._INTL("{1} changed Forme!",pokemon.Name));
         ...
         if (chosen>=0) {
           Pokemon poke2=Game.GameData.Trainer.party[chosen];
...
BattleUseOnPokemon.Add(Items.REVIVE, (pokemon, battler, scene) => {
   if (pokemon.HP>0) {
     ...
     for (int i = 0; i < Game.GameData.Player.Party.Length; i++) {
       if (Game.GameData.Player.Party[i]==pokemon) {
...
BattleUseOnBattler.Add(Items.GUARD_SPEC, (item, battler, scene) => {
   ...
   if (battler.OwnSide.Mist>0) {
     ...
     //if (!scene.pbIsOpposing(attacker.index)) { //Create new Delegate for attacker?
...
BattleUseOnBattler.Add(Items.POKE_DOLL, (item, battler, scene) => {
   Combat.Battle battle=battler.battle;
...
BattleUseOnBattler.Add(Items.POKE_BALL, (item, battler, scene) => {
   ...
   //if (battle.pbPlayer().party.Length>=6 && !Game.GameData.PC.full) {
   if (battle.pbPlayer().party.GetCount()>=Game.GameData.Features.LimitPokemonPartySize && !Game.GameData.Player.PC.hasSpace()) {
...
BattleUseOnBattler.Add(Items.BEAST_BALL, (item, battler, scene) => {
   ...
   if (battle.pbPlayer().party.Length>=6 && !Game.GameData.Player.PC.hasSpace()) { //Game.GameData.PC.full
     scene.pbDisplay(Game._INTL("There is no room left in the PC!"));
     return false;
   }
   return true;
...
*/
 ...
 {
   //if (!Terrain.isIce(Game.GameData.GamePlayer.terrain_tag)) {		// Shouldn't count down if on ice
   //  if (Game.GameData.RepelSteps>0) {
   //    Game.GameData.RepelSteps-=1;
   //    if (Game.GameData.RepelSteps<=0) {
   //      Game.pbMessage(Game._INTL("Repel's effect wore off..."));
   ...
   //      if (ret>0) Item.pbUseItem(Game.GameData.Bag,ret);
   //    }
   //  }
   //}
 }
