...
public class DialogBoxManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        DialogBoxTexture = DialogBox.GetComponent<Image>();
        //link the DialogBoxText variable to the Text Component
        DialogBoxText = DialogBox.transform.Find("BoxText").GetComponent<Text>();
        // //link the DialogBoxTextShadow variable to the Text Component #DEPRECATED
        ...
        DialogBoxBorder = DialogBox.transform.Find("BoxBorder").GetComponent<Image>();
        animArrow = DialogBox.transform.Find("animArrow").GetComponent<Text>();
		// 
        // //link the ChoiceBox variable to the Object
        ChoiceBox = gameObject.transform.Find("ChoiceBox").gameObject;
        // //link the ChoiceBoxTexture variable to the Object's texture
        ChoiceBoxTexture = ChoiceBox.GetComponent<Image>();
        ...
        ChoiceBoxBorder = ChoiceBox.transform.Find("BoxBorder").GetComponent<Image>();
        // //link the ChoiceBoxText variable to the Text Component
        ChoiceBoxText = ChoiceBoxBorder.transform.Find("BoxText").GetComponent<Text>();
        // //link the ChoiceBoxSelect variable to the Texture Component
        ChoiceBoxSelect = ChoiceBoxBorder.transform.Find("Arrow").GetComponent<Text>();
    ...
    public string getLongerChoice(string[] choices) {

        string str = "";
        str = choices[0];

        for (int i = 0; i < choices.Length; i++){
            if (i >0 && choices[i].ToCharArray().Length>str.ToCharArray().Length){
                str=choices[i];
                Debug.Log("Longer");
            }
        }
    ...
    public void drawChoiceBox(string[] choices)
    {
        ...
        if (getLongerChoice(choices).ToCharArray().Length*8.5f>83.81f){
            boxWidth = getLongerChoice(choices).ToCharArray().Length*8.5f;
        }
        else{
            boxWidth = 83.81f;
        }

        if (getLongerChoice(choices).ToCharArray().Length*11f>66f){
            borderWidth = getLongerChoice(choices).ToCharArray().Length*8.5f - 17f;//getLongerChoice(choices).ToCharArray().Length*11f;
        }
        else{
            borderWidth = 66.8f;
        }
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            //ChoiceBoxTexture.rectTransform.sizeDelta = new Vector2(boxWidth,choices.Length*32f);
            ChoiceBoxBorder.rectTransform.sizeDelta = new Vector2(borderWidth,choices.Length*23f);
    ...
    private IEnumerator drawChar(char character, float secPerChar)
    {
        ...
        else
        {
            ...
            DialogBoxString = DialogBoxText.text;
            //DialogBoxTextShadow.text = DialogBoxText.text;
        }
        yield return new WaitForSeconds(0.00001f); //wait for (seconds per character print) before printing the next
    }
    ...
    {
        chosenIndex = 0; //0 is the vertically lowest choice
        bool selected = false;
        while (!selected)
        {
            
            ChoiceBoxSelect.rectTransform.DOAnchorPos(new Vector2(-40.55f,-24f-(16f*chosenIndex)),0.1f);
            if (Input.GetButtonDown("Select"))
            {
                selected = true;
            }
    ...
    public void animContinueON(){
        animArrow.gameObject.SetActive(true);
    }

    public void animContinueOFF(){
