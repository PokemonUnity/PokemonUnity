...
public class Function
{
	...
	public class PokeBattle_Move_075 : PokeBattle_Move
	{
		public object pbModifyDamage(int damagemult, Pokemon attacker, Pokemon opponent)
		{
			if ((int)new Move((Moves)opponent.effects.TwoTurnAttack).Function == 0xCB)
			{ // Dive
				return (int)Math.Round(damagemult * 2.0f);
			}
			return damagemult;
	...
	public class PokeBattle_Move_076 : PokeBattle_Move
	{
		public object pbModifyDamage(int damagemult, Pokemon attacker, Pokemon opponent)
		{
			...
			if ((int)new Move((Moves)opponent.effects.TwoTurnAttack).Function == 0xCA)
			{ // Dig
	...
	public class PokeBattle_Move_07A : PokeBattle_Move
	{
		...
		public object pbShowAnimation(Moves id, Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			if (opponent.damagestate.Critical || this.doubled)
			{
				return base.pbShowAnimation(id, attacker, opponent, 1, alltargets, showanimation); // Charged anim;
	...
	public class PokeBattle_Move_083 : PokeBattle_Move
	{
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			...
			if (opponent.damagestate.CalcDamage > 0)
			{
				...
				{
					if ((int)this.battle.choices[attacker.Partner.Index].Action == 1)
					{ // Will use a move
	...
	public class PokeBattle_Move_087 : PokeBattle_Move
	{
		...
		public object pbModifyType(Types type, Pokemon attacker, Pokemon opponent)
		{
	...
	{
		public Types pbModifyType(Types type, Pokemon attacker, Pokemon opponent)
		{
	...
	{
		public object pbDisplayUseMessage(Pokemon attacker)
		{
	...
	public class PokeBattle_Move_106 : PokeBattle_Move
	{
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			...
			else if ((int)attacker.effects.FirstPledge == 0x108)
			{   // Water Pledge
				object ret = base.pbEffect(attacker, opponent, hitnum, alltargets, showanimation);
				if (opponent.damagestate.CalcDamage > 0)
				{
	...
	public class PokeBattle_Move_107 : PokeBattle_Move
	{
		public object pbOnStartUse(Pokemon attacker)
		{
			...
			{       // Water Pledge
					//battle.pbDisplay(_INTL("The two moves have become one! It's a combined move!"))
				this.doubledamage = true;
				if ((int)attacker.effects.FirstPledge == 0x108)
				{   // Water Pledge
					this.overridetype = true;
				}
			}
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			...
			// Combined move's effect
			if ((int)attacker.effects.FirstPledge == 0x106)
			{   // Grass Pledge
				object ret = base.pbEffect(attacker, opponent, hitnum, alltargets, showanimation);
				if (opponent.damagestate.CalcDamage > 0)
				{
	...
	public class PokeBattle_Move_108 : PokeBattle_Move
	{
		public object pbOnStartUse(Pokemon attacker)
		{
			this.doubledamage = false; this.overridetype = false;
			if ((int)attacker.effects.FirstPledge == 0x106 ||   // Grass Pledge
			   (int)attacker.effects.FirstPledge == 0x107)
			{      // Fire Pledge
				   //battle.pbDisplay(_INTL("The two moves have become one! It's a combined move!"))
				this.doubledamage = true;
				if ((int)attacker.effects.FirstPledge == 0x106)
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			...
			else if ((int)attacker.effects.FirstPledge == 0x107)
			{   // Fire Pledge
				object ret = base.pbEffect(attacker, opponent, hitnum, alltargets, showanimation);
				if (opponent.damagestate.CalcDamage > 0)
				{
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				if ((int)new Move((Moves)poke.effects.TwoTurnAttack).Function == 0xC9 || // Fly
				   ...
				   (int)new Move((Moves)poke.effects.TwoTurnAttack).Function == 0xCE)
				{    // Sky Drop
		...
		public override object pbEffect(Pokemon attacker, Pokemon opponent, byte hitnum = 0, byte? alltargets = null, bool showanimation = true)
		{
				...
				!opponent.effects.Roost)
			{
				...
				bool showmsg = (opponent.hasType(Types.FLYING) ||
						 opponent.hasWorkingAbility(Abilities.LEVITATE));
				if ((int)new Move((Moves)opponent.effects.TwoTurnAttack).Function == 0xC9 || // Fly
				   (int)new Move((Moves)opponent.effects.TwoTurnAttack).Function == 0xCC)
				{    // Bounce
...
public abstract class PokeBattle_Move : IPokeBattle_Move
{
	...
	public virtual object pbModifyType(Types type, Pokemon atk, Pokemon opp)
	{
		return Types.NONE;
	}
