...
public class DialogBoxHandler : MonoBehaviour {
	...
	public void drawChoiceBox(){ //No parametres means simply Yes/No
		drawChoiceBox(0);}
	public void drawChoiceBox(int customYOffset){ //No other parametres means simply Yes/No
	...
	public void drawChoiceBoxNo(){ //set default position to 0
		ChoiceBox.SetActive(true);
		ChoiceBoxTexture.texture = Resources.Load<Texture>("Frame/choice"+PlayerPrefs.GetInt("frameStyle"));
	...
	private IEnumerator drawChar(char character, float secPerChar){
		if (character.Equals('\\')){	//   \ is used to designate line breaks
			DialogBoxText.text += "\n";
	...
	public IEnumerator drawText(string textLine){
		...
		for (int i = 0; i < textLine.Length; i++){
			if(chars[i] == '{'){ //extended operator
				if(chars[i+1] == 'p' || chars[i+1] == 'P'){ //player name
	...
	public IEnumerator drawTextSilent(string textLine){
		...
		char[] chars = textLine.ToCharArray();
		for (int i = 0; i < textLine.Length; i++){
			if (chars[i].Equals('\\')){	//   \ is used to designate line breaks
				DialogBoxText.text += "\n";
				DialogBoxTextShadow.text = DialogBoxText.text; 
			}
			else{
			...
			if(Time.deltaTime < secPerChar){
				yield return new WaitForSeconds(secPerChar);} //wait for (seconds per character print) before printing the next
			else {
				i += 1;
				if(i < textLine.Length){ //if not at the end, repeat and wait double time
					if (chars[i].Equals('\\')){	//   \ is used to designate line breaks
						DialogBoxText.text += "\n";
						DialogBoxTextShadow.text = DialogBoxText.text; 
					}
	...
	public IEnumerator scrollText(){
		...
		while (DialogBoxText.pixelOffset.y < textPosDestination){
			if (Mathf.RoundToInt(DialogBoxText.pixelOffset.y) == textPosDestination-5){ //if text is about to spill over the top
	...
	public IEnumerator choiceNavigate(){ //No parametres means simply Yes/No
	...
	public IEnumerator choiceNavigateNo(){ //For when No needs to be default;
		chosenIndex = 0; 
		bool selected = false;
		while (!selected){
			if(Input.GetButtonDown("Select")){
