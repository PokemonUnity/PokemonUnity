...
public class AudioHandler : MonoBehaviour {
	...
	void Update () {
		...
		if (increment < 1 && fading){
			...
			if(source.isPlaying){ //don't start new song until unpaused.
				source.Stop();
				source.volume = PlayerPrefs.GetFloat("musicVolume");
	...
	public void playBGM(AudioClip BGM, int loopStartTimeSamples){
		...
		else if(source.clip != BGM){ //if BGM is not already playing
			if (!fading){	//if fading is off, then begin to fade
				increment = 0;
				fading = true;
			}
