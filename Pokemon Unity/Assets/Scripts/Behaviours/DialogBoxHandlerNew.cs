...
public class DialogBoxHandlerNew : MonoBehaviour
{
        ...
        if (chars.Length > 0) {
            ...
            if (chars[0] == '\\') {
                ...
                switch (chars[1]) {
                    case ('p'): //Player
                        if (secPerChar > 0)
                            yield return StartCoroutine(DrawWord(SaveData.currentSave.playerName, large, bold, italic, secPerChar));
                        else
                ...
                switch (chars[1]) {
                    ...
                    case ('f'):
                        if (secPerChar > 0)
                            yield return StartCoroutine(DrawWord(SaveData.currentSave.PC.boxes[0][0].getName(), large, bold, italic, secPerChar));
                        else
    ...
    public void UndrawNameBox() => nameBox.gameObject.SetActive(false);

    #endregion

    #region Choices Boxes
    ...
    public IEnumerator DrawChoiceBoxNo() {
        yield return StartCoroutine(DrawChoiceBox(0));
    }
