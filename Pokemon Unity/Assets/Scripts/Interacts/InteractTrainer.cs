	...
	private IEnumerator turnAtRandom(){
		...
		while(!recentlyDefeated){
			while(!busy){
				...
				while(!turnableDirections[direction]){ //ensure trainer is facing valid direction
					updateDirection(Random.Range(0,4));}
		...
		if(currentSlope <= 1 && destinationSlope <= 1){
			...
			if(yDistance <= currentSlope || yDistance <= destinationSlope){
				...
				if(destinationTileTag == 1){
					...
					if(trainerSurfing){ //if a surf trainer, normal tiles are impassible
	...
	private IEnumerator interact(){
		if(PlayerMovement.player.setCheckBusyWith(this.gameObject)){
			...
			if (xDistance >= Mathf.Abs(zDistance)){ //Mathf.Abs() converts zDistance to a positive always.
				updateDirection(3);}			//this allows for better accuracy when checking orientation.
