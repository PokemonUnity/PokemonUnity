...
public class InteractPush : MonoBehaviour {
	...
	public IEnumerator bump(){
		if (Player.strength){
			if(Player.setCheckBusyWith(this.gameObject)){
				...
				if (checkDestination(movement)){ //if destination is clear
	...
	private bool checkDestination(Vector3 movement){
		...
		if (Mathf.RoundToInt(movement.x) != 0){ // moving right or left
			direction = 1;
			destinationPosition2 = destinationPosition1 + new Vector3(0,0,1);
		}
		else if (Mathf.RoundToInt(movement.z) != 0){ //moving up or down
		...
		if(closestIndex != -1){
			...
			if (hitColliders.Length > 0){
				for (int i = 0; i < hitColliders.Length; i++){
					if(hitColliders[i].name.ToLowerInvariant().Contains("object")){ //if hits object
			...
			if (currentObjectCollider == null){ //if both positions are free
