	...
	void OnTriggerEnter(Collider other){
		if(other.name == "Player_Transparent"){
			if(PlayerMovement.player.busyWith != trainer.gameObject){
				...
				if(playerX == Mathf.Round(transform.position.x)){ //player is up or down from
					if(playerZ > transform.position.z){			//up
						playerLocation = 0;}
					else{										//down
						playerLocation = 2;}
				}
				else if(playerZ == Mathf.Round(transform.position.z)){//player is right or left of
					if(playerX > transform.position.x){			//right
						playerLocation = 1;}
					else{										//left
						playerLocation = 3;}
				}
