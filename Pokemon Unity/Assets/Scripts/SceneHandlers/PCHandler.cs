...
public class PCHandler : MonoBehaviour {
	...
	private IEnumerator packParty(int currentPosition){
		...
		if(hole < 5){
			...
			while(hole < 4){ //until the hole is either at the end of the party, or not there.
	...
	private IEnumerator endOfParty(int currentPosition){
		...
		Vector2[] partyPositions = new Vector2[]{
			...
			new Vector2(partyIcons[4].pixelInset.x,partyIcons[4].pixelInset.y), new Vector2(partyIcons[5].pixelInset.x,partyIcons[5].pixelInset.y)};
		if(icon > 0){
			while(SaveData.currentSave.PC.boxes[0][icon-1] == null && icon > 1){ //if the previous spot is free, and is not first spot
	...
	private IEnumerator putDownPokemon(int currentBoxID, int currentPosition){
		...
		if(SaveData.currentSave.PC.boxes[currentBoxID][currentPosition] == null || originalSpot){
			...
			yield return StartCoroutine(moveCursor(new Vector2(cursor.pixelInset.x,cursor.pixelInset.y+10)));
			carrying = false;
			if(currentBoxID != 0){ //fully heal if depositing into PC
							...
							if(!carrying){
								...
								if(chosenIndex == 5){
									...
									if(!string.IsNullOrEmpty(currentPokemon.getHeldItem())){
										...
										else if(itemChosenIndex == 1){	//Take
											Dialog.undrawChoiceBox();
								...
								else if(chosenIndex == 2){		//WITHDRAW
									if(SaveData.currentSave.PC.boxes[0][5] != null){ //if party is full
										Dialog.drawDialogBox();
						...
						if(cursorMode == CursorMode.Standard){
							...
							if(!carrying){
								...
								while(chosenIndex != 0){
									...
									if(chosenIndex == 5){			//PICK UP
										if(SaveData.currentSave.PC.boxes[0][1] != null){ //if there is more than one pokemon in the party
										...
										if(SaveData.currentSave.PC.boxes[0][1] != null){ //if there is more than one pokemon in the party
											...
											if(targetPosition >= 30){ //if box is full
												Dialog.drawDialogBox();
