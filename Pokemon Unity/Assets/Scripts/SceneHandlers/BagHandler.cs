...
public class BagHandler : MonoBehaviour {
	...
	private void updateScrollBar(){
		...
		if(itemsLength != maxView){ //to prevent dividing by 0.
		...
		while(running){
			if(Input.GetAxisRaw("Vertical") > 0){
				...
				if(!switching && currentItemList.Length > 0){
					...
					else{ //display options for Pokemon on their own.
					...
					if(!inParty  && currentItemList.Length > 0){
						...
						if(currentPosition[currentScreen] == selectedPosition && currentTopPosition[currentScreen] == selectedTopPosition){ 
							...
							Dialog.undrawDialogBox();
							Dialog.undrawChoiceBox();
							if(chosenIndex == 3){ //USE (unique key item) //unique effects not yet implemented

							}
							if(chosenIndex == 2){
								if(selectedItem.getItemType() == ItemData.ItemType.KEY){ //REGISTER (key item)
									Dialog.drawDialogBox();
									...
									if(selectedItem.getItemEffect() == ItemData.ItemEffect.UNIQUE ){
										...
										if(selectedItem.getItemType() != ItemData.ItemType.TM){
											...
											chosenIndex = Dialog.chosenIndex;
											Dialog.undrawChoiceBox();
											if(chosenIndex == 0){ //If No is chosen
					...
					else if(inParty){ //display options for using selected item on Pokemon
						Pokemon currentPokemon = SaveData.currentSave.PC.boxes[0][partyPosition];
	...
	private IEnumerator runItemEffect(ItemData selectedItem, Pokemon currentPokemon, bool booted){
		if(selectedItem.getItemEffect() == ItemData.ItemEffect.HP){					//HP
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.PP){			//PP
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.STATUS){		//STATUS
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.EV){			//EV
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.EVOLVE){		//EVOLVE
			if(currentPokemon.canEvolve("Stone,"+selectedItem.getName())){
				...
				if(oldID != currentPokemon.getID()){ //if evolved
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.TM){			//TM
			...
			else if(currentPokemon.CanLearnMove(selectedItem.getName())){ //check if can learn move
				if(!booted){
					Dialog.drawDialogBox(2);
					SfxHandler.Play(tmBootupClip);
					yield return StartCoroutine(Dialog.drawTextSilent("Booted up a TM."));
		...
		else if(selectedItem.getItemEffect() == ItemData.ItemEffect.UNIQUE){		//UNIQUE
