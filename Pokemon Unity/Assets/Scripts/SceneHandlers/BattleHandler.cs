        ...
        while (running)
        {
            ...
            while (runState)
            {
                ...
                else if (Input.GetButtonDown("Select"))
                {
                    ...
                    if(SaveData.currentSave.debugMode) canMegaEvolve = true; //give debug option to win battle
                    if (taskPosition == 1)
                    {
                        updateCurrentTask(1);
                        ...
                        while (currentTask == 1)
                        {
                            ...
                            else if (Input.GetButtonDown("Select"))
                            {
                                if (movePosition == 0)
                                {
                                    //if mega evolution selected (mega evolution not yet implemented, used for debug atm)
                                    SfxHandler.Play(selectClip);
                                    if(SaveData.currentSave.debugMode) //double check to make sure the player is in debug mode
                                    {
                                        commandMove[currentPokemon] = MoveDatabase.getMove("Struggle");
                                        victor = 0; //give the player a victory after struggling
                                        runState = false;
                                    }
                                    else updateCurrentTask(0);
                                }
                                else if (movePosition == 3)
                                {
                                    //if back selected
