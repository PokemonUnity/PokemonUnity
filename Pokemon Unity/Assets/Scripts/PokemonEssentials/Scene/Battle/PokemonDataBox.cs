...
namespace PokemonUnity.Interface.UnityEngine
{
	...
	namespace Themes
	{
		...
		/// </remarks>
		/// ToDo: Should use SafariDatabox as abstract base...
		[RequireComponent(typeof(global::UnityEngine.UI.Image))]
		//[ExecuteInEditMode]
		public abstract class PokemonDataBoxBase : SafariDataBox, IPokemonDataBox, IGameObject
		{
			public IBattlerIE battler { get; protected set; }
			...
			/// </summary>
			public global::UnityEngine.UI.Slider sliderExp;
			public global::UnityEngine.UI.Image spriteItem, spriteStatus, spriteCaught, spriteFillHP, spriteFillExp, shiny, primal;
			//public global::UnityEngine.UI.Text currentHP, slash, maxHP, Name, level, gender;
			//public TMPro.TextMeshProUGUI Name, HPValue, level;
			//protected UnityEngine.Color colorFillExp;
			//protected UnityEngine.UI.Image panelbg;
			...
			public abstract IPokemonDataBox initialize(IBattler battler, bool doublebattle, IViewport viewport = null);
			//{
			//	Core.Logger?.LogDebug(message: "Run: {0}.{1}", GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name);
			//	base.initialize(viewport);
			//	this.battler = battler as IBattlerIE;
			...
			//	this.visible = false;
			//	this.z = 50;*/
			//	//primal = Resources.Load<global::UnityEngine.Sprite>("null");
			//	shiny?.gameObject.SetActive(battler.IsShiny);
			//	spriteCaught?.gameObject.SetActive(false);
			//	spriteItem?.gameObject.SetActive(false);
			//	refreshExpLevel();
		...
		[RequireComponent(typeof(global::UnityEngine.UI.Image))]
		[ExecuteInEditMode]
		public class PokemonDataBoxUnity : PokemonDataBoxBase, IPokemonDataBox, IGameObject
		{
			public global::UnityEngine.UI.Text currentHP, slash, maxHP, Name, level, gender;

			public override IPokemonDataBox initialize(IBattler battler, bool doublebattle, IViewport viewport = null)
			{
				Core.Logger?.LogDebug(message: "Run: {0}.{1}", GetType().Name, System.Reflection.MethodBase.GetCurrentMethod().Name);
				base.initialize(viewport);
				this.battler = battler as IBattlerIE;
			...
			public override void refresh()
			{
				...
				if (@showhp)
				{
					//string hpstring = string.Format("{1: 2d}/{2: 2d}", this.HP, @battler.TotalHP);
					//textpos.Add(new TextPosition(hpstring, @spritebaseX + 188, 48, true, base_, shadow));
					maxHP.text = @battler.TotalHP.ToString(); //sliderHP.maxValue.ToString(); //Set text under hp to match slider maxHealth
					currentHP.text = this.HP.ToString(); //sliderHP.value.ToString(); //Set text under hp to match slider currentHealth
				}
				//DrawTextPositions(this.bitmap,textpos);
				//IList<ITextPosition> imagepos = new List<ITextPosition>();
				//ToDo: Uncomment below if UI is setup for generation features/gimmick
		...
		[RequireComponent(typeof(global::UnityEngine.UI.Image))]
		[ExecuteInEditMode]
		public class PokemonDataBoxTMPro : PokemonDataBoxBase, IPokemonDataBox, IGameObject
		{
			public global::UnityEngine.UI.Image gender;
			public TMPro.TextMeshProUGUI Name, HPValue, level;
			public global::UnityEngine.Sprite genderMale, genderFemale; //ToDo: Move to a different parent class
			...
			public override void refresh()
			{
				...
				if (@battler.displayGender > 0) //switch (@battler.displayGender)
				{
					...
					{
						//textpos.Add(new TextPosition(Game._INTL("♂"), genderX, 6, false, new Color(48, 96, 216), shadow));
						gender.sprite = genderMale; // Male
						//break;
					}
					else //	case 1: // Female
					{
						//textpos.Add(new TextPosition(Game._INTL("♀"), genderX, 6, false, new Color(248, 88, 40), shadow));
						gender.sprite = genderFemale; // Female
						//break;
					}
				}
				else
				{
					gender.sprite = Resources.Load<global::UnityEngine.Sprite>("null");
					gender.gameObject.SetActive(false);
				}
				//DrawTextPositions(this.bitmap,textpos);
				//SetSmallFont(this.bitmap);
