...
namespace PokemonUnity.Legacy
{
	public class MapSettings : MonoBehaviour
	{
		...
		public UnityEngine.Sprite getBattleBase(int currentTag = 0)
		{
			float time = System.DateTime.Now.Hour + ((float) System.DateTime.Now.Minute / 60f);
			string timeString = "";
			if (time >= 20 || time < 3.5f)
			...
			{
				//return with timestring attached
				return Resources.Load<UnityEngine.Sprite>("BattleBackgrounds/Bases/" + checkString + timeString);
			}

			//else return without timestring
			return Resources.Load<UnityEngine.Sprite>("BattleBackgrounds/Bases/" + checkString);
		}
