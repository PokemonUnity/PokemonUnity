...
namespace PokemonUnity.Legacy
{
	public static class SaveLoad
	{
		...
		public static void Save()
		{
			if (SaveData.currentSave != null)
			{
				if (SaveData.currentSave.getFileIndex() >= 0 && SaveData.currentSave.getFileIndex() < savedGames.Length)
				{
					...
					FileStream file = File.Create(UnityEngine.Application.persistentDataPath + "/playerData.pkud");
		...
		public static bool Load()
		{
			...
			if (File.Exists(UnityEngine.Application.persistentDataPath + "/playerData.pkud"))
			{
				BinaryFormatter bf = new BinaryFormatter();
				FileStream file = File.Open(UnityEngine.Application.persistentDataPath + "/playerData.pkud", FileMode.Open);
