...
using PokemonUnity.Legacy.Data;
...
namespace PokemonUnity.Legacy
{
	...
	public class PC
	{
		...
		{
			return addPokemon(acquiredPokemon, false);
		}

		//Add a new pokemon. If pokemon could not be added return false.
		public bool addPokemon(Pokemon acquiredPokemon, bool onlyPC)
		{
			//attempt to add to party first. pack the party array if space available.
			if (hasSpace(0) && !onlyPC)
			{
				packParty();
				boxes[0][boxes[0].Length - 1] = acquiredPokemon;
				packParty();
				SaveData.currentSave.pokedex.SetSeen(Pokedex.findPokedexIndex(acquiredPokemon.getID()));
				return true;
			}
