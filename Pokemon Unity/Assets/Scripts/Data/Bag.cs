...
public class Bag {
	...
	public bool addItem(string itemName, int amount){
		...
		if(index == -1){ //item does not exist in bag, add it to the end
			index = getLength();
			order[index] = name;
		}
	...
	public bool removeItem(string itemName, int amount){
		...
		string name = ItemDatabase.getItem(itemName).getName(); //ensures that the name is correct
		int index = getIndexOf(name);
		if(index == -1){ //item does not exist in bag
			return false;
		}
		index = ItemDatabase.getIndexOf(order[index]);
	...
	private string[] getItemTypeArray(ItemData.ItemType itemType, bool allSellables){
		...
		for(int i = 0; i < length; i++){
			if(!allSellables){
				if(ItemDatabase.getItem(order[i]).getItemType() == itemType){ //if correct ItemType
				...
				if(ItemDatabase.getItem(order[i]).getItemType() == ItemData.ItemType.ITEM || 
				   ...
				   ItemDatabase.getItem(order[i]).getItemType() == ItemData.ItemType.BERRY){ //if correct ItemType
					result[resultPos] = order[i];
					resultPos += 1;
				}
			}
	...
	public string[] getBattleTypeArray(ItemData.BattleType battleType){
		...
		//cycle through order, adding all correct ItemTypes to result
		for(int i = 0; i < length; i++){
			if(ItemDatabase.getItem(order[i]).getBattleType() == battleType){ //if correct ItemType
				result[resultPos] = order[i];
				resultPos += 1;
			}
		}
