...
public static class PokemonDatabase
{
	/// <summary>
	/// Author: joseka1234
	/// Using a dictionary to acces data with complexity O(1)
	/// </summary>
	private static Dictionary<int,PokemonData> dicPokedex;
	...
	{
		dicPokedex = new Dictionary<int, PokemonData> ();
		for (int i = 0; i < pokedex.Length; i++) {
			if (pokedex [i] == null) {
				dicPokedex.Add (0, null);
			} else {
				dicPokedex.Add (pokedex [i].getID (), pokedex [i]);
			}
		}
	...
	/// <param name="ID">The ID of the pokemon that we want to find.</param>
	/// Author: joseka1234
	/// Note: With the change of a simple array to a Dictionary we can access to the pokemon data with complexity O(1).
	/// This have a contrapart too because now we are storing a Dictionary that consume more memory but
	/// I think that is better to use a bit more of memory and access it faster.
	/// Note2: Now I'm using a function to pass the old array to the new dictionary but the ideal situation is to
	/// initialize the dictionary by hand to avoid the unnecessary loop and double memory usage.
	public static PokemonData getPokemon (int ID)
	{
		if (dicPokedex == null || dicPokedex.Count == 0) {
			InitializePokedex ();
		}
		return dicPokedex [ID];		
	}

	/// <summary>
	/// New function to get the level experience
	/// </summary>
	/// <returns>The level exp.</returns>
	/// <param name="levelingRate">Leveling rate.</param>
	/// <param name="currentLevel">Current level.</param>
	/// Author: joseka1234
	/// Note: Using this function instead of the previous one we have less acces to memory so the game will be fast
	/// because memory acces is an important bottle neck.
	/// Instead of acces to multiple arrays that use memory allocation, I propose to use some functions with complexity
	/// O(1) so the getLevelExp function will be faster than the previous one.
	public static int getLevelExp (PokemonData.LevelingRate levelingRate, int currentLevel)
	{
		...
		switch (levelingRate) {
		case PokemonData.LevelingRate.ERRATIC:
			return getExperienceErratic (currentLevel);
			break;
		case PokemonData.LevelingRate.FAST:
			return getExperienceFast (currentLevel);
			break;
		case PokemonData.LevelingRate.MEDIUMFAST:
			return getExperienceMediumFast (currentLevel);
			break;
		case PokemonData.LevelingRate.MEDIUMSLOW:
			return getExperienceMediumSlow (currentLevel);
			break;
		case PokemonData.LevelingRate.SLOW:
			return getExperienceSlow (currentLevel);
			break;
		case PokemonData.LevelingRate.FLUCTUATING:
			return getExperienceFluctuating (currentLevel);
			break;
		}
		// A return of -1 is an error because we can't access to a different type of leveling rate
		return -1;
	}

	#region Experience functions

	// This functions are from https://bulbapedia.bulbagarden.net/wiki/Experience

	private static int getExperienceErratic (int currentLevel)
	{
		if (currentLevel <= 50) {
			return (int)((Mathf.Pow (currentLevel, 3) * (100 - currentLevel)) / 50);
		} else if (currentLevel > 50 && currentLevel <= 68) {
			return (int)((Mathf.Pow (currentLevel, 3) * (150 - currentLevel)) / 100);
		} else if (currentLevel > 68 && currentLevel <= 98) {
			return (int)((Mathf.Pow (currentLevel, 3) * Mathf.Floor ((1911 - 10 * currentLevel) / 3)) / 500);
		} else if (currentLevel > 98 && currentLevel <= 100) {
			return (int)((Mathf.Pow (currentLevel, 3) * (160 - currentLevel)) / 100);
		}
		// A return of -1 is an error because we cant have a level greater than 100
		return -1;
	}

	private static int getExperienceFast (int currentLevel)
	{
		return (int)((4 * Mathf.Pow (currentLevel, 3)) / 5);
	}

	private static int getExperienceMediumFast (int currentLevel)
	{
		return (int)(Mathf.Pow (currentLevel, 3));
	}

	private static int getExperienceMediumSlow (int currentLevel)
	{
		return (int)(((6 / 5) * Mathf.Pow (currentLevel, 3)) - (15 * Mathf.Pow (currentLevel, 2)) + (100 * currentLevel) - 140);
	}

	private static int getExperienceSlow (int currentLevel)
	{
		return (int)((5 * Mathf.Pow (currentLevel, 3)) / 4);
	}

	private static int getExperienceFluctuating (int currentLevel)
	{
		if (currentLevel <= 15) {
			return (int)(Mathf.Pow (currentLevel, 3) * ((Mathf.Floor ((currentLevel + 1) / 3) + 24) / 50));
		} else if (currentLevel > 15 && currentLevel <= 36) {
			return (int)(Mathf.Pow (currentLevel, 3) * ((currentLevel + 14) / 50));
		} else if (currentLevel > 36 && currentLevel <= 100) {
			return (int)(Mathf.Pow (currentLevel, 3) * ((Mathf.Floor (currentLevel / 2) + 32) / 50));
		}
		// A return of -1 is an error because we cant have a level greater than 100
		return -1;
	}
