...
namespace PokemonUnity.Legacy {
	public partial class OLD_BattleHandler : MonoBehaviour {
			...
			/*if (ally == null && target <= 2 || ally != null && target == 0)
			{
				if (pokemon[target].Status == Status.FAINT)
				{
					if (ally == null)
					{
						...
						{
							isLowHP = false;
							BackgroundMusicHandler.Singleton.PlayOverlay(battleBGM, battleBGMLoopStart, sample);
						}
					}
					else
					{
				...
				else if (pokemon[target].PercentHP < 0.25f)
				{
					...
					{
						isLowHP = true;
						sample = BackgroundMusicHandler.Singleton.GetSample();
						BackgroundMusicHandler.Singleton.PlayOverlay(lowHpBGM, lowHpBGMLoopStart);
					}
				}
				else
				{
	...
	//back
					...
					if (victor == 0)
					{
						if (trainerBattle)
						{
							...
							{
								BackgroundMusicHandler.Singleton.PlayOverlay(defaultTrainerVictoryBGM, defaultTrainerVictoryBGMLoopStart);
							}
							else
							{
								BackgroundMusicHandler.Singleton.PlayOverlay(trainer.victoryBGM, trainer.victorySamplesLoopStart);
							}
						...
						else //wild battle
						{
							if (trainer.victoryBGM == null)
							{
								BackgroundMusicHandler.Singleton.PlayOverlay(defaultWildVictoryBGM, defaultWildVictoryBGMLoopStart);
							}
							else
							{
								BackgroundMusicHandler.Singleton.PlayOverlay(trainer.victoryBGM, trainer.victorySamplesLoopStart);
			...
			if (victor == 1) {
				...
				StartCoroutine(ScreenFade.Singleton.Fade(false, 1f));
				BackgroundMusicHandler.Singleton.ResumeMain(1.4f, PlayerMovement.Singleton.newMap.GetBackgroundMusic());
			}
			...
			if (victor == 1) {
				if (!healedOnDefeat) {
					GlobalVariables.Singleton.Respawn();
				}
			}
			if (BattleTravelling != null)
				StopCoroutine(BattleTravelling);
		...
		public void UpdateRPC(bool isTrainer, Trainer trainer) {
			...
			//    detail: PlayerMovement.Singleton.accessedMapSettings.mapName+" - "+dayTime, 
			//    state: "In "+((isTrainer) ? "Trainer" : "Wild")+" Battle",
			//    largeKey: PlayerMovement.Singleton.accessedMapSettings.RPCImageKey,
			//    largeText: "",
			//    smallKey: "pokeball", 
