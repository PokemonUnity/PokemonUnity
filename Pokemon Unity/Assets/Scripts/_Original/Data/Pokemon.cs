...
public class Pokemon
{
    ...
    private static Sprite[] GetAnimFromID_(string folder, int ID, Gender gender, bool isShiny)
    {
        ...
        {
            animation[i] = Sprite.Create(animation[i].texture, animation[i].rect, new Vector2(0.5f, 0), 16f);
        }
        
        return animation;
    }

    private static Sprite[] GetAnimFromID2_(string folder, int ID, Gender gender, bool isShiny)
    {
        Sprite[] animation;
        ...
        {
            //Attempt to load Female Variant
            animation = Resources.LoadAll<Sprite>(folder + (isShiny ? "Shiny" : "") + "/" + convertLongID(ID) + "f" + shiny + "/");
            if (animation.Length == 0)
            {
                Debug.LogWarning("Female Variant NOT Found");
                //Attempt to load Base Variant (possibly Shiny)
        ...
        {
            //Attempt to load Base Variant (possibly Shiny)
            animation = Resources.LoadAll<Sprite>(folder  + (isShiny ? "Shiny" : "")  + "/" + convertLongID(ID) + shiny + "/");
        }
        if (animation.Length == 0 && isShiny)
        {
            Debug.LogWarning("Shiny Variant NOT Found");
        ...
        if (animation.Length == 0)
        {
            animation = Resources.LoadAll<Sprite>("PokemonSprites/Missing");
        }
    ...
    public Sprite[] GetNewSprite(bool getLight)
    {
    ...
    public static Sprite[] GetIconsSpriteFromID(int ID, bool isShiny)
    {
        string shiny = (isShiny) ? "s" : "";
        Sprite[] icons = Resources.LoadAll<Sprite>("PokemonIcons/icon" + convertLongID(ID) + shiny);
        if (icons == null)
        ...
        if (icons.Length == 0)
        {
            ...
            icons = Resources.LoadAll<Sprite>("PokemonIcons/icon001");
        }
    ...
    public static Sprite[] GetNewSpriteFromID(int ID, bool isShiny, bool getLight)
    {
        string shiny = (isShiny) ? "s" : "";
        string light = (getLight) ? "Lights/" : "";
        Sprite[] spriteSheet;
        if (getLight)
        {
            spriteSheet = Resources.LoadAll<Sprite>("OWPokemonSprites/" + light + convertLongID(ID) + shiny + "_0");
        }
        else
        {
            spriteSheet = Resources.LoadAll<Sprite>("OWPokemonSprites/" + convertLongID(ID) + shiny + "_0");
        }
        ...
        if (spriteSheet.Length == 0)
        {
            ...
            spriteSheet = Resources.LoadAll<Sprite>("OWPokemonSprites/" + light + convertLongID(ID) + "_0");
        }
        if (spriteSheet.Length == 0)
        {
            //No Light found OR No Sprite found, return 8 blank sprites
            ...
            spriteSheet =  Resources.LoadAll<Sprite>("OWPokemonSprites/001_0");
        }
        
        for (int i = 0; i < spriteSheet.Length; ++i)
        {
            spriteSheet[i] = Sprite.Create(spriteSheet[i].texture, spriteSheet[i].rect, new Vector2(0.5f, 0), 16f);
        }
