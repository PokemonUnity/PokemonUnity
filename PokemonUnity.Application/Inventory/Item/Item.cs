...
namespace PokemonUnity.Inventory
{
	...
	public static partial class Item {
		...
		public static bool IsHiddenMove (Moves move) { //ToDo: Move to Game Class
		...
		public static ItemPockets? GetPocket(Items item) {
		...
		public static bool IsHiddenMachine (Items item) {
		...
		public static bool IsMail (Items item) {
			return Kernal.ItemData.ContainsKey(item) && (ItemData.IsLetter(item)); //[ITEMTYPE]==1 || Kernal.ItemData[item][ITEMTYPE]==2
		}

		public static bool IsSnagBall (Items item) {
			return Kernal.ItemData.ContainsKey(item) && (ItemData.IsPokeBall(item) || Kernal.ItemData[item].Pocket == ItemPockets.POKEBALL);// || //[ITEMTYPE]==3
		...
		public static bool IsKeyItem (Items item) {
		...
		public static bool IsEvolutionStone (Items item) {
		...
		public static bool IsMegaStone (Items item) {   // Does NOT include Red Orb/Blue Orb
		...
		public static bool IsMulch (Items item) {
		...
		public static void ChangeLevel(IPokemon pokemon,int newlevel,IScene scene) {
			...
			if (pokemon.Level>newlevel) {
				...
				pokemon.calcStats();
				scene.Refresh();
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("{1} was downgraded to Level {2}!",pokemon.Name,pokemon.Level));
				...
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("{1}'s level remained unchanged.",pokemon.Name));
			} else {
				int attackdiff=pokemon.ATK;
				int defensediff=pokemon.DEF;
				int speeddiff=pokemon.SPE;
				...
				pokemon.calcStats();
				scene.Refresh();
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("{1} was elevated to Level {2}!",pokemon.Name,pokemon.Level));
				attackdiff=pokemon.ATK-attackdiff;
				defensediff=pokemon.DEF-defensediff;
				speeddiff=pokemon.SPE-speeddiff;
				spatkdiff=pokemon.SPA-spatkdiff;
		...
		public static int ItemRestoreHP(IPokemon pokemon,int restorehp) {
		...
		public static bool HPItem(IPokemon pokemon,int restorehp,IScene scene) {
		...
		public static bool BattleHPItem(IPokemon pokemon,IBattler battler,int restorehp,IScene scene) {
			if (pokemon.HP<=0 || pokemon.HP==pokemon.TotalHP || pokemon.isEgg) {
		...
		public static int JustRaiseEffortValues(IPokemon pokemon,Stats ev,int evgain) {
			int totalev=0;
			for (int i = 0; i < 6; i++) {
		...
		public static int RaiseEffortValues(IPokemon pokemon,Stats ev,int evgain=10,bool evlimit=true) {
		...
		public static bool RaiseHappinessAndLowerEV(IPokemon pokemon,IScene scene,Stats ev,string[] messages) {
		...
		public static int RestorePP(IPokemon pokemon,int move,int pp) {
		...
		public static int BattleRestorePP(IPokemon pokemon,IBattler battler,int move,int pp) {
		...
		public static bool BikeCheck() {
			if (Game.GameData.Global.surfing ||
				(!Game.GameData.Global.bicycle && Terrain.onlyWalk(Game.GameData is PokemonEssentials.Interface.Field.IGameField f ? f.GetTerrainTag() : (Terrains?)null))) {
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("Can't use that here."));
				return false;
			}
			if (Game.GameData.GamePlayer.HasDependentEvents()) {
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("It can't be used when you have someone with you."));
				return false;
			}
			if (Game.GameData.Global.bicycle) {
				//if (Game.GameData.GetMetadata(Game.GameData.GameMap.map_id,MetadataBicycleAlways)) {
				if (Game.GameData is PokemonEssentials.Interface.Field.IGameMetadataMisc e ? e.GetMetadata(Game.GameData.GameMap.map_id).Map.BicycleAlways : false) {
					if (Game.GameData is IGameMessage m) m.Message(Game._INTL("You can't dismount your Bike here."));
					return false;
				}
				return true;
			} else {
				//bool? val=Game.GameData.GetMetadata(Game.GameData.GameMap.map_id,MetadataBicycle);
				bool? val=Game.GameData is PokemonEssentials.Interface.Field.IGameMetadataMisc e0 ? e0.GetMetadata(Game.GameData.GameMap.map_id).Map.Bicycle : (bool?)null;
				//if (val == null) val=Game.GameData.GetMetadata(Game.GameData.GameMap.map_id,MetadataOutdoor);
				if (val == null) val=Game.GameData is PokemonEssentials.Interface.Field.IGameMetadataMisc e1 ? e1.GetMetadata(Game.GameData.GameMap.map_id).Map.Outdoor : false;
				if (val == null) {
					if (Game.GameData is IGameMessage m) m.Message(Game._INTL("Can't use that here."));
					return false;
				}
		...
		public static IGameCharacter ClosestHiddenItem() {
		...
		public static void UseKeyItemInField(Items item) { //Move to game class
			if (!ItemHandlers.triggerUseInField(item)) {
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("Can't use that here."));
			}
		}

		public static bool SpeciesCompatible (Pokemons species,Moves move) {
			//bool ret=false;
		...
		public static int ForgetMove(IPokemon pokemon,Moves moveToLearn) {
			int ret=-1;
			if (Game.GameData is IGameSpriteWindow w) w.FadeOutIn(99999, block: () => {
		...
		public static bool LearnMove(IPokemon pokemon,Moves move,bool ignoreifknown=false,bool bymachine=false) {
			...
			if (pokemon.isEgg && !Core.DEBUG) {
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("{1} can't be taught to an Egg.",movename));
				return false;
			}
			if (pokemon is IPokemonShadowPokemon p && p.isShadow) {
				if (Game.GameData is IGameMessage m) m.Message(Game._INTL("{1} can't be taught to this Pokémon.",movename));
				return false;
			}
			string pkmnname=pokemon.Name;
			for (int i = 0; i < 4; i++) {
				if (pokemon.moves[i].id==move) {
					if (!ignoreifknown && Game.GameData is IGameMessage m) m.Message(Game._INTL("{1} already knows {2}.",pkmnname,movename));
					return false;
				}
				if (pokemon.moves[i].id==0) {
					pokemon.moves[i]=new Attack.Move(move);
					if (Game.GameData is IGameMessage m) m.Message(Game._INTL("\\se[]{1} learned {2}!\\se[MoveLearnt]",pkmnname,movename));
					return true;
				}
			}
			do { //;loop
				if (Game.GameData is IGameMessage m0) m0.Message(Game._INTL("{1} wants to learn the move {2}.",pkmnname,movename));
				if (Game.GameData is IGameMessage m1) m1.Message(Game._INTL("However, {1} already knows four moves.",pkmnname));
				if (Game.GameData is IGameMessage m2 && m2.ConfirmMessage(Game._INTL("Should a move be deleted and replaced with {1}?",movename))) {
					m2.Message(Game._INTL("Which move should be forgotten?"));
					...
					if (forgetmove>=0) {
						...
						m2.Message(Game._INTL("\\se[]1,\\wt[16] 2, and\\wt[16]...\\wt[16] ...\\wt[16] ... Ta-da!\\se[balldrop]"));
						m2.Message(Game._INTL("\\se[]{1} forgot how to use {2}. And... {1} learned {3}!\\se[MoveLearnt]",pkmnname,oldmovename,movename));
						return true;
					} else if (m2.ConfirmMessage(Game._INTL("Give up on learning the move {1}?",movename))) {
						m2.Message(Game._INTL("{1} did not learn {2}.",pkmnname,movename));
						return false;
					}
				} else if (Game.GameData is IGameMessage m3 && m3.ConfirmMessage(Game._INTL("Give up on learning the move {1}?",movename))) {
					m3.Message(Game._INTL("{1} did not learn {2}.",pkmnname,movename));
					return false;
				}
			} while (true);
		}

		public static bool CheckUseOnPokemon(Items item,IPokemon pokemon,IScreen screen) {
		...
		public static bool ConsumeItemInBattle(PokemonBag bag,Items item) {
			if (item!=0 && Kernal.ItemData[item].Flags.Consumable && //!=3 disappear after use
				...
				return Game.GameData.Bag.DeleteItem(item);
			}
			return false;
		}
		...
		/*public static bool UseItemOnPokemon(Items item,IPokemon pokemon,IScene scene) {
			//if (Kernal.ItemData[item][ITEMUSE]==3 || Kernal.ItemData[item][ITEMUSE]==4) {		// TM or HM
			if (Item.IsMachine(item)) {
				...
				if (pokemon.isShadow) { //? rescue false
					Game.GameData.Message(Game._INTL("Shadow Pokémon can't be taught any moves."));
				} else if (!pokemon.isCompatibleWithMove(machine)) {
					Game.GameData.Message(Game._INTL("{1} and {2} are not compatible.",pokemon.Name,movename));
					Game.GameData.Message(Game._INTL("{1} can't be learned.",movename));
				} else {
					if (IsHiddenMachine(item)) {
						Game.GameData.Message(Game._INTL("\\se[accesspc]Booted up an HM."));
						Game.GameData.Message(Game._INTL(@"It contained {1}.\1",movename));
					} else {
						Game.GameData.Message(Game._INTL("\\se[accesspc]Booted up a TM."));
						Game.GameData.Message(Game._INTL(@"It contained {1}.\1",movename));
					}
					if (Game.GameData.ConfirmMessage(Game._INTL("Teach {1} to {2}?",movename,pokemon.Name))) {
						if (LearnMove(pokemon,machine,false,true)) {
							if (IsTechnicalMachine(item) && !Core.INFINITETMS) Game.GameData.Bag.DeleteItem(item);
							return true;
						}
					}
				}
				...
				if (ret && Kernal.ItemData[item].Flags.Consumable) {		//[ITEMUSE]==1 Usable on Pokémon, consumed
					Game.GameData.Bag.DeleteItem(item);
				}
				if (Game.GameData.Bag.Quantity(item)<=0) {
					Game.GameData.Message(Game._INTL("You used your last {1}.",Game._INTL(item.ToString(TextScripts.Name))));
				}
				return ret;
			}
			Game.GameData.Message(Game._INTL("Can't use that on {1}.",pokemon.Name));
			return false;
		}

		public static int UseItem(PokemonBag bag,Items item,IScene bagscene=null) {
			//bool found=false;
			//if (Kernal.ItemData[item][ITEMUSE]==3 || Kernal.ItemData[item][ITEMUSE]==4) {		// TM or HM
			if (Item.IsMachine(item)) {
			Moves machine=Kernal.MachineData[(int)item].Move;
			...
			if (Game.GameData.Trainer.pokemonCount==0) {
				Game.GameData.Message(Game._INTL("There is no Pokémon."));
				return 0;
			}
			string movename=Game._INTL(machine.ToString(TextScripts.Name));
			if (IsHiddenMachine(item)) {
			...
			if (!Game.GameData.ConfirmMessage(Game._INTL("Teach {1} to a Pokémon?",movename))) {
				return 0;
			} else if (Game.MoveTutorChoose(machine,null,true)) {
			...
			if (IsEvolutionStone(item)) {
				...
				bool elig=(Evolution.CheckEvolution(pkmn,item).Length>0);
			...
			if (Game.GameData is IGameSpriteWindow w) w.FadeOutIn(99999, block: () => {
				...
				screen.StartScene(Game._INTL("Use on which Pokémon?"),false,annot);
				do { //;loop
			...
			return ret ? 1 : 0;
				...
				switch (intret) {
					...
					case 4: // Item used, end screen and consume item
						...
						break;
					default:
						Game.GameData.Message(Game._INTL("Can't use that here."));
						return 0;
				}
			} else {
				Game.GameData.Message(Game._INTL("Can't use that here."));
		...
		public static Items ChooseItem(int var=0,params Items[] args) {
			Items ret=0; //int?
		...
		/// <returns></returns>
		public static Items ChooseItemFromList(string message,int variable,params Items[] args) {
			List<string> commands=new List<string>();
			List<Items> itemid=new List<Items>();
			foreach (Items item in args) {
				//if (hasConst(Items,item)) {
					Items id=(Items)item;
					if (Game.GameData.Bag.Quantity(id)>0) {
			...
			itemid.Add(0);
			int ret=Game.GameData is IGameMessage m ? m.Message(message,commands.ToArray(),-1) : -1;
		...
		private static void TopRightWindow(string text) {
			...
			if (Game.GameData is IGameAudioPlay a) a.PlayDecisionSE();
			do { //;loop
				//Graphics?.update();
				Input.update();
				window.update();
	...
	public static partial class ItemHandlers {
		...
		/*private static void RegisterItemHandlers() {
			...
			UseFromBag.Add(Items.SUPER_REPEL, () => { return Repel(Items.SUPER_REPEL,200); });
			UseFromBag.Add(Items.MAX_REPEL, () => { return Repel(Items.MAX_REPEL,250); });
			...
			UseFromBag.Add(Items.BICYCLE, () => {
				//next BikeCheck ? 2 : 0;
				return BikeCheck() ? ItemUseResults.CloseBagNotConsumed : ItemUseResults.NotUsed;
				//return ItemUseResults.NotUsed;
			});
			...
			UseFromBag.Add(Items.OLD_ROD, () => {
				Terrains terrain=FacingTerrainTag();
			...
			UseInField.Add(Items.DOWSING_MACHINE, () => {//item == Items.ITEM_FINDER || item == Items.DOWSING_MCHN ||
				@event=Game.GameData is IItemCheck i && i.ClosestHiddenItem();
				if (@event == null) {
			...
			UseOnPokemon.Add(Items.FIRE_STONE, (item, pokemon, scene) => {
				...
				Pokemons newspecies=Pokemons.NONE; //Evolution.CheckEvolution(pokemon,item)[0];
				if (newspecies<=0) {
					scene.Display(Game._INTL("It won't have any effect."));
					//next false;
					return false;
			...
			UseOnPokemon.Add(Items.FULL_RESTORE, (item, pokemon, scene) => {
				if (pokemon.HP<=0 || (pokemon.HP==pokemon.TotalHP && pokemon.Status==0)) {
					...
					int hpgain=Item.ItemRestoreHP(pokemon,pokemon.TotalHP-pokemon.HP);
					pokemon.HealStatus();
					scene.Refresh();
					if (hpgain>0) {
					scene.Display(Game._INTL("{1}'s HP was restored by {2} points.",pokemon.Name,hpgain));
			...
			UseOnPokemon.Add(Items.ENERGY_POWDER, (item, pokemon, scene) => {
				if (Item.HPItem(pokemon,50,scene)) {
			...
			UseOnPokemon.Add(Items.ENERGY_ROOT, (item, pokemon, scene) => {
				if (Item.HPItem(pokemon,200,scene)) {
			...
			UseOnPokemon.Add(Items.ETHER, (item, pokemon, scene) => {
				int move=scene.ChooseMove(pokemon,Game._INTL("Restore which move?"));
				if (move>=0) {
					if (Item.RestorePP(pokemon,move,10)==0) {
						scene.Display(Game._INTL("It won't have any effect."));
						//next false;
						return false;
					} else {
			...
			UseOnPokemon.Add(Items.MAX_ETHER, (item, pokemon, scene) => {
				int move=scene.ChooseMove(pokemon,Game._INTL("Restore which move?"));
				if (move>=0) {
					if (Item.RestorePP(pokemon,move,pokemon.moves[move].TotalPP-pokemon.moves[move].PP)==0) {
						scene.Display(Game._INTL("It won't have any effect."));
						return false;
					} else {
						scene.Display(Game._INTL("PP was restored."));
			...
			UseOnPokemon.Add(Items.ELIXIR, (item, pokemon, scene) => {
				...
				for (int i = 0; i < pokemon.moves.Length; i++) {
					pprestored+=Item.RestorePP(pokemon,i,10);
				}
				if (pprestored==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
			...
			UseOnPokemon.Add(Items.MAX_ELIXIR, (item, pokemon, scene) => {
				int pprestored=0;
				for (int i = 0; i < pokemon.moves.Length; i++) {
					pprestored+=Item.RestorePP(pokemon,i,pokemon.moves[i].TotalPP-pokemon.moves[i].PP);
				}
				if (pprestored==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
			...
			UseOnPokemon.Add(Items.PP_UP, (item, pokemon, scene) => {
				int move=scene.ChooseMove(pokemon,Game._INTL("Boost PP of which move?"));
				if (move>=0) {
			...
			UseOnPokemon.Add(Items.HP_UP, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.HP)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Refresh();
			...
			UseOnPokemon.Add(Items.PROTEIN, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.ATTACK)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
			...
			UseOnPokemon.Add(Items.IRON, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.DEFENSE)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
			...
			UseOnPokemon.Add(Items.CALCIUM, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPATK)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
			...
			UseOnPokemon.Add(Items.ZINC, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPDEF)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
			...
			UseOnPokemon.Add(Items.CARBOS, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPEED)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
			...
			UseOnPokemon.Add(Items.HEALTH_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.HP,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Refresh();
			...
			UseOnPokemon.Add(Items.MUSCLE_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.ATTACK,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Display(Game._INTL("{1}'s Attack increased.",pokemon.Name));
			...
			UseOnPokemon.Add(Items.RESIST_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.DEFENSE,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Display(Game._INTL("{1}'s Defense increased.",pokemon.Name));
			...
			UseOnPokemon.Add(Items.GENIUS_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPATK,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Display(Game._INTL("{1}'s Special Attack increased.",pokemon.Name));
			...
			UseOnPokemon.Add(Items.CLEVER_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPDEF,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Display(Game._INTL("{1}'s Special Defense increased.",pokemon.Name));
			...
			UseOnPokemon.Add(Items.SWIFT_WING, (item, pokemon, scene) => {
				if (Item.RaiseEffortValues(pokemon,Stats.SPEED,1,false)==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
				} else {
					scene.Display(Game._INTL("{1}'s Speed increased.",pokemon.Name));
			...
			UseOnPokemon.Add(Items.RARE_CANDY, (item, pokemon, scene) => {
				if (pokemon.Level>=Core.MAXIMUMLEVEL || (pokemon.isShadow)) { //rescue false
					...
					Item.ChangeLevel(pokemon,pokemon.Level+1,scene);
					scene.HardRefresh();
					return true;
				}
			});
			...
			UseOnPokemon.Add(Items.POMEG_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.HP,new string[] {
			...
			UseOnPokemon.Add(Items.KELPSY_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.ATTACK,new string[] {
			...
			UseOnPokemon.Add(Items.QUALOT_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.DEFENSE,new string[] {
			...
			UseOnPokemon.Add(Items.HONDEW_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.SPATK,new string[] {
			...
			UseOnPokemon.Add(Items.GREPA_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.SPDEF,new string[] {
			...
			UseOnPokemon.Add(Items.TAMATO_BERRY, (item, pokemon, scene) => {
				return Item.RaiseHappinessAndLowerEV(pokemon,scene,Stats.SPEED,new string[] {
			...
			UseOnPokemon.Add(Items.ABILITY_CAPSULE, (item, pokemon, scene) => {
				...
				if (scene.Confirm(Game._INTL("Would you like to change {1}'s Ability to {2}?",
			...
			BattleUseOnPokemon.Add(Items.POTION, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,20,scene);
			});

			BattleUseOnPokemon.Add(Items.SUPER_POTION, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,50,scene);
			});

			BattleUseOnPokemon.Add(Items.HYPER_POTION, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,200,scene);
			});

			BattleUseOnPokemon.Add(Items.MAX_POTION, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,pokemon.TotalHP-pokemon.HP,scene);
			});
			...
			BattleUseOnPokemon.Add(Items.SODA_POP, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,60,scene);
			});

			BattleUseOnPokemon.Add(Items.LEMONADE, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,80,scene);
			});

			BattleUseOnPokemon.Add(Items.MOOMOO_MILK, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,100,scene);
			});

			BattleUseOnPokemon.Add(Items.ORAN_BERRY, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,10,scene);
			});

			BattleUseOnPokemon.Add(Items.SITRUS_BERRY, (pokemon, battler, scene) => {
				return Item.BattleHPItem(pokemon,battler,(int)Math.Floor(pokemon.TotalHP/4f),scene);
			});
			...
			BattleUseOnPokemon.Add(Items.REVIVE, (pokemon, battler, scene) => {
				if (pokemon.HP>0) {
					...
					pokemon.HP=(int)Math.Floor(pokemon.TotalHP/2f);
					//Item.ItemRestoreHP(pokemon,(int)Math.Floor(pokemon.TotalHP/2f));
					pokemon.HealStatus();
			...
			BattleUseOnPokemon.Add(Items.ENERGY_POWDER, (pokemon, battler, scene) => {
				if (Item.BattleHPItem(pokemon,battler,50,scene)) {
					pokemon.ChangeHappiness(HappinessMethods.POWDER);
					return true;
				}
				return false;
			...
			BattleUseOnPokemon.Add(Items.ENERGY_ROOT, (pokemon, battler, scene) => {
				if (Item.BattleHPItem(pokemon,battler,200,scene)) {
					pokemon.ChangeHappiness(HappinessMethods.ENERGYROOT);
					return true;
				}
				return false;
			...
			BattleUseOnPokemon.Add(Items.ETHER, (pokemon, battler, scene) => {
				int move=scene.ChooseMove(pokemon,Game._INTL("Restore which move?"));
				if (move>=0) {
					if (Item.BattleRestorePP(pokemon,battler,move,10)==0) {
						scene.Display(Game._INTL("It won't have any effect."));
						return false;
					} else {
						scene.Display(Game._INTL("PP was restored."));
			...
			BattleUseOnPokemon.Add(Items.MAX_ETHER, (pokemon, battler, scene) => {
				int move=scene.ChooseMove(pokemon,Game._INTL("Restore which move?"));
				if (move>=0) {
					if (Item.BattleRestorePP(pokemon,battler,move,pokemon.moves[move].TotalPP-pokemon.moves[move].PP)==0) {
						scene.Display(Game._INTL("It won't have any effect."));
						return false;
					} else {
						scene.Display(Game._INTL("PP was restored."));
			...
			BattleUseOnPokemon.Add(Items.ELIXIR, (pokemon, battler, scene) => {
				int pprestored=0;
				for (int i = 0; i < pokemon.moves.Length; i++) {
					pprestored+=Item.BattleRestorePP(pokemon,battler,i,10);
				}
				if (pprestored==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
			...
			BattleUseOnPokemon.Add(Items.MAX_ELIXIR, (pokemon, battler, scene) => {
				int pprestored=0;
				for (int i = 0; i < pokemon.moves.Length; i++) {
					pprestored+=Item.BattleRestorePP(pokemon,battler,i,pokemon.moves[i].TotalPP-pokemon.moves[i].PP);
				}
				if (pprestored==0) {
					scene.Display(Game._INTL("It won't have any effect."));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ATTACK, (item, battler, scene) => {
				...
				if (battler.CanIncreaseStatStage(Combat.Stats.ATTACK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ATTACK,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ATTACK_2, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.ATTACK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ATTACK,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ATTACK_3, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.ATTACK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ATTACK,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ATTACK_6, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.IncreaseStatWithCause(Combat.Stats.ATTACK,6,battler,Game._INTL(item.ToString(TextScripts.Name)))) {
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_DEFENSE, (item, battler, scene) => { //item == Items.X_DEFEND ||
				...
				if (battler.CanIncreaseStatStage(Combat.Stats.DEFENSE,battler,false)) {
					battler.IncreaseStat(Combat.Stats.DEFENSE,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false  ;
			...
			BattleUseOnBattler.Add(Items.X_DEFENSE_2, (item, battler, scene) => { //item == Items.XDEFEND2 ||
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.DEFENSE,battler,false)) {
					battler.IncreaseStat(Combat.Stats.DEFENSE,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_DEFENSE_3, (item, battler, scene) => { //item == Items.XDEFEND3 ||
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.DEFENSE,battler,false)) {
					battler.IncreaseStat(Combat.Stats.DEFENSE,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_ATK, (item, battler, scene) => { //item == Items.X_SPECIAL ||
				...
				if (battler.CanIncreaseStatStage(Combat.Stats.SPATK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPATK,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_ATK_2, (item, battler, scene) => { //item == Items.XSPECIAL2 ||
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPATK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPATK,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_ATK_3, (item, battler, scene) => { //item == Items.XSPECIAL3 ||
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPATK,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPATK,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_DEF, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPDEF,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPDEF,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_DEF_2, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPDEF,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPDEF,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_DEF_3, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPDEF,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPDEF,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SP_DEF_6, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.IncreaseStatWithCause(Combat.Stats.SPDEF,6,battler,Game._INTL(item.ToString(TextScripts.Name)))) {
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SPEED, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPEED,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPEED,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SPEED_2, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPEED,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPEED,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SPEED_3, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.SPEED,battler,false)) {
					battler.IncreaseStat(Combat.Stats.SPEED,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_SPEED_6, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.IncreaseStatWithCause(Combat.Stats.SPEED,6,battler,Game._INTL(item.ToString(TextScripts.Name)))) {
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ACCURACY, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.ACCURACY,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ACCURACY,1,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ACCURACY_2, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.ACCURACY,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ACCURACY,2,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.X_ACCURACY_3, (item, battler, scene) => {
				...
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.CanIncreaseStatStage(Combat.Stats.ACCURACY,battler,false)) {
					battler.IncreaseStat(Combat.Stats.ACCURACY,3,battler,true);
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false  ;
			...
			BattleUseOnBattler.Add(Items.X_ACCURACY_6, (item, battler, scene) => {
				string playername=battler.battle.Player().name;
				scene.Display(Game._INTL("{1} used the {2}.",playername,Game._INTL(item.ToString(TextScripts.Name))));
				if (battler.IncreaseStatWithCause(Combat.Stats.ACCURACY,6,battler,Game._INTL(item.ToString(TextScripts.Name)))) {
					return true;
				} else {
					scene.Display(Game._INTL("But it had no effect!"));
					return false;
			...
			BattleUseOnBattler.Add(Items.GUARD_SPEC, (item, battler, scene) => {
				...
				if (battler.OwnSide.Mist>0) {
					...
					if (!battler.battle.OwnedByPlayer(battler.Index)) { //if player's pokemon...
			...
			BattleUseOnBattler.Add(Items.POKE_BALL, (item, battler, scene) => {
				...
				if (!battler.Opposing1.isFainted() && !battler.Opposing2.isFainted()) {
					if (!(Game.GameData as IItemCheck).IsSnagBall(item)) { //battle.IsSnagBall(item)
				...
				if (battle.Player().party.GetCount()>=Features.LimitPokemonPartySize && !Player.PC.hasSpace()) {
			...
			BattleUseOnBattler.Add(Items.BEAST_BALL, (item, battler, scene) => {
				...
				if (battle.Player().party.Length>=6 && !Player.PC.hasSpace()) { //PC.full
					scene.Display(Game._INTL("There is no room left in the PC!"));
					return false;
				}
				return true;
