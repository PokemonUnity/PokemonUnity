...
namespace PokemonUnity.Avatar
{
	public class Character :  IGameCharacter
	{
		...
		//public Game_Map Game_Map					{ get { return map.HasValue ? map.Value : Game.Game_Map; } }
		...
		#region Constructor
		public Character(IGameMap map=null) {
			initialize(map);
		}
		public virtual IGameCharacter initialize(IGameMap map=null) {
			this.map=map;
			@id = 0;
			@x = 0;
			@y = 0;
		...
		public bool moving { get {
			return (@real_x != @x*4*Game.Game_Map.TILEWIDTH || @real_y != @y*4*Game.Game_Map.TILEHEIGHT);
		} }
		...
		public bool passableEx (float x,float y,int d,bool strict=false) {
			...
			foreach (IGameCharacter @event in this.map.events.Values) {
		...
		public void triggerLeaveTile() {
			if (@oldX != null && @oldY != null && @oldMap != null &&
				(@oldX!=this.x || @oldY!=this.y || @oldMap!=this.map.map_id)) {
				//Events.onLeaveTile.trigger(this,this,@oldMap,@oldX,@oldY);
			}
			@oldX=this.x;
			@oldY=this.y;
			@oldMap=this.map.map_id;
		}

		public virtual void moveto(float x,float y) {
			this.x = x % this.map.width;
			this.y = y % this.map.height;
			@real_x = @x * Game.Game_Map.realResX;
			@real_y = @y * Game.Game_Map.realResY;
			@prelock_direction = 0;
			triggerLeaveTile();
		}

		public float screen_x { get {
			return (@real_x - this.map.display_x + 3) / 4 + (Game.Game_Map.TILEWIDTH/2);
		} }

		public float screen_y { get {
			float y = (@real_y - this.map.display_y + 3) / 4 + (Game.Game_Map.TILEHEIGHT);
			if (jumping) {
				float n = Math.Abs(@jump_count - @jump_peak);
				return y - (@jump_peak * @jump_peak - n * n) / 2;
			}
			return y;
		} }

		public float screen_y_ground() {
			return (@real_y - this.map.display_y + 3) / 4 + (Game.Game_Map.TILEHEIGHT);
		}
		...
		public int screen_z(int height = 0) {
			...
			int z = (int)((@real_y - this.map.display_y + 3) / 4 + 32);
			if (@tile_id > 0) {
				return z + this.map.priorities[@tile_id] * 32;
			}
			//  Add z if height exceeds 32
			return z + ((height > 32) ? 31 : 0);
		}

		public virtual int bush_depth { get {
		...
		public Terrains terrain_tag { get {
			return (Terrains)this.map.terrain_tag(@x, @y);
		} }

		#region Updating stuff ###############################################################
		public virtual void update() {
			if (Game.GameData.GameTemp.in_menu) return;
			if (jumping) { update_jump();
			} else if (moving) { update_move();
			} else { update_stop();
			}
			if (@wait_count > 0) {
				@wait_count -= 1;
		...
		public void update_jump() {
			@jump_count -= 1;
			@real_x = (@real_x * @jump_count + @x * Game.Game_Map.realResX) / (@jump_count + 1);
			@real_y = (@real_y * @jump_count + @y * Game.Game_Map.realResY) / (@jump_count + 1);
			if (!jumping && !moving) {
		...
		public void update_move() {
			float distance = (float)Math.Pow(2, @move_speed);
			int realResX=Game.Game_Map.realResX;
			int realResY=Game.Game_Map.realResY;
		...
		public void move_type_custom() {
			...
			while (@move_route_index < @move_route.list.Count) { //@move_route.list.size
				PokemonEssentials.Interface.RPGMaker.Kernal.IMoveCommand command = @move_route.list[@move_route_index];
				if (command.code == 0) {
				...
				switch (command.code) {
					//case 27:
					//	Game.GameData.GameSwitches[command.parameters[0]] = true;
					//	this.map.need_refresh = true;
					//	break;
					//case 28:
					//	Game.GameData.GameSwitches[command.parameters[0]] = false;
					//	this.map.need_refresh = true;
					//	break;
					//case 29:
					...
					//	break;
					//case 44:
					//	if (Game.GameData is IGameAudioPlay ga) ga.pbSEPlay(command.parameters[0]);
					//	break;
					case 45:
		...
		public virtual void move_down(bool turn_enabled = true) {
			...
			if (passable(@x, @y, 2)) {
				...
				if (this is IGamePlayer ge) ge.check_event_trigger_touch(@x, @y+1);
			}
		}
		...
		public virtual void move_left(bool turn_enabled = true) {
			...
			if (passable(@x, @y, 4)) {
				...
				if (this is IGamePlayer ge) ge.check_event_trigger_touch(@x-1, @y);
			}
		}
		...
		public virtual void jump(float x_plus,float y_plus) {
			...
			float new_x = @x + x_plus;
			float new_y = @y + y_plus;
			float oldX=@x;
			float oldY=@y;
		...
		public virtual void turnGeneric(int dir) {
			if (!@direction_fix) {
				...
				@direction=dir;
				@stop_count=0;
				if (dir != oldDirection && this is IGameEvent ge) {
					ge.pbCheckEventTriggerAfterTurning();
				}
			}
		}
