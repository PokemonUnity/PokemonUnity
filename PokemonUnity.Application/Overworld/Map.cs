...
namespace PokemonUnity
{
	...
	public partial class Game_Map : PokemonEssentials.Interface.IGameMap, IGameMapOrgBattle
	{
		...
		public IList<IEncounterPokemon> encounter_list { get {
		...
		public void autoplayAsCue() {
			if (@map.autoplay_bgm) {
				if (Game.IsNight) { //&& FileTest.audio_exist("Audio/BGM/"+ @map.bgm.name+ "n")
					if (Game.GameData is PokemonEssentials.Interface.Field.IGameField f) f.CueBGM(@map.bgm.name+"n",1.0f,@map.bgm.volume,@map.bgm.pitch);
				} else {
					if (Game.GameData is PokemonEssentials.Interface.Field.IGameField f) f.CueBGM(@map.bgm,1.0f);
				}
			}
			if (@map.autoplay_bgs) {
				if (Game.GameData is PokemonEssentials.Interface.IGameAudioPlay a) a.BGSPlay(@map.bgs);
			}
		}
		/// <summary>
		...
		public void autoplay() {
			if (@map.autoplay_bgm) {
				if (Game.IsNight) { //&& FileTest.audio_exist("Audio/BGM/"+ @map.bgm.name+ "n")
					if (Game.GameData is PokemonEssentials.Interface.IGameAudioPlay a) a.BGMPlay(@map.bgm.name+"n",@map.bgm.volume,@map.bgm.pitch);
				} else {
					if (Game.GameData is PokemonEssentials.Interface.IGameAudioPlay a) a.BGMPlay(@map.bgm);
				}
			}
			if (@map.autoplay_bgs) {
				if (Game.GameData is PokemonEssentials.Interface.IGameAudioPlay a) a.BGSPlay(@map.bgs);
...
//				}
			...
			if (@map_id != 0) {
				foreach (int i in new int[] { 2, 1, 0 }) { //foreach z axis in map
					...
					if (tile_id != null && Terrain.isBridge(@terrain_tags[tile_id.Value]) &&
						Game.GameData.Global != null && Game.GameData.Global.bridge==0 && !countBridge) continue;
					if (tile_id == null) {
						return 0;
		...
		public bool in_range (IGameCharacter @object) {
			if (Game.GameData.PokemonSystem.tilemap==2) return true;
			float screne_x = display_x - 4*32*4;
			float screne_y = display_y - 4*32*4;
			int screne_width = display_x + Game.GameData.Graphics.width*4 + 4*32*4;
			int screne_height = display_y + Game.GameData.Graphics.height*4 + 4*32*4;
	...
	//public partial class Game_Map {
		...
		protected int _display_x;
		/// <summary>
		/// display x-coordinate * 128
		/// </summary>
		public int display_x {
			get { return _display_x; }
			set {
				_display_x=value;
				//if (GetMetadata(this.map_id,MetadataSnapEdges)) {
				if (Game.GameData is IGameMetadataMisc gmm && gmm.GetMetadata(this.map_id).Map.SnapEdges) {
					float max_x = (this.width - Game.GameData.Graphics.width*1.0f/Game_Map.TILEWIDTH) * Game_Map.realResX;
					@display_x = (int)Math.Max(0, Math.Min(@display_x, max_x));
				}
				if (Game.GameData.MapFactory != null) Game.GameData.MapFactory.setMapsInRange();
			}
		}

		protected int _display_y;
		/// <summary>
		/// display y-coordinate * 128
		/// </summary>
		public int display_y {
			get { return _display_y; }
			set {
				@display_y=value;
				//if (GetMetadata(this.map_id,MetadataSnapEdges)) {
				if (Game.GameData is IGameMetadataMisc gmm && gmm.GetMetadata(this.map_id).Map.SnapEdges) {
					float max_y = (this.height - Game.GameData.Graphics.height*1.0f/Game_Map.TILEHEIGHT) * Game_Map.realResY;
					@display_y = (int)Math.Max(0, Math.Min(@display_y, max_y));
				}
				if (Game.GameData.MapFactory != null) Game.GameData.MapFactory.setMapsInRange();
			}
		}
