	...
	public static class GameExtension
	{
		public static PokemonUnity.Game SetScenes(this PokemonUnity.Game game, params PokemonEssentials.Interface.Screen.IScene[] scenes)
		{
			foreach (PokemonEssentials.Interface.Screen.IScene scene in scenes)
			{
				...
				//	//Note: I dont believe application will be in a state of managing multiple game instance UIs from one source 
				//	//Maybe if doing game cast to sync with handheld game device for head-to-head as use case; but will device have seperate install?
				//	// (i.e. one exe handles different simultainious gameplays with UI in different states)
