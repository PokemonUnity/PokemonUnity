...
namespace PokemonUnity.Monster
{
	...
	public partial class Pokemon : PokemonEssentials.Interface.PokeBattle.IPokemonMegaEvolution
	{
		...
		public bool IsPrimal { get { Forms f = Kernal.PokemonFormsData[Species][form].Id; return f == Forms.GROUDON_PRIMAL || f == Forms.KYOGRE_PRIMAL; } }
		...
		public string megaName() {
			...
			if (IsMega) v = Kernal.PokemonFormsData[Species][form].Id.ToString(TextScripts.Name);
	...
	public static partial class MultipleForms {
		public static int? getMegaForm(IPokemon pokemon) {
		...
		public static int? getPrimalForm(IPokemon pokemon) {
		...
		/*public int getMegaForm(IPokemon pokemon) {
			#region XY Mega Evolution
			if (pokemon.Species == Pokemons.VENUSAUR) {
