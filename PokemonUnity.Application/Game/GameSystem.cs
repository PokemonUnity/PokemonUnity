...
namespace PokemonUnity
{
	public partial class GameSystem : IGameSystem
	{
		...
		public void bgm_play_internal2(string name, float volume, float pitch, int position)
		{
			...
			//try
			//{
				if (Game.GameData.Audio is IAudio audio) audio.bgm_play(name, vol, pitch, position);
			//}
			//catch //ArgumentError;
			//{
			//	AudioManager.AudioHandler.bgm_play(name, vol, pitch);
			//}
		}
		/// <inheritdoc/>
		public void bgm_play_internal(IAudioBGM bgm, int position)
		{
			...
			{
				if (Game.FileTest.audio_exist("Audio/BGM/" + bgm.name))
				{
			...
			else
			{
				...
				@playing_bgm = null;
				//if (defaultBGM == null) AudioManager.AudioHandler.bgm_stop();
				if (defaultBGM == null && Game.GameData.Audio is IAudio audio) audio.bgm_stop();
			}
			if (defaultBGM != null)
			{
		...
		{
			int pos = 0;
			if (Game.GameData is IAudio ga) pos = ga.bgm_position; //rescue 0;
			if (fadetime > 0.0)
			{
		...
		public void bgm_stop()
		{
			...
			@playing_bgm = null;
			if (defaultBGM == null && Game.GameData is IAudio ga) ga.bgm_stop();
		}
		/// <inheritdoc/>
		public void bgm_fade(float time)
		{
			...
			@playing_bgm = null;
			if (defaultBGM == null && Game.GameData is IAudio ga) ga.bgm_fade((int)Math.Floor(time * 1000));
		}
		...
		public void setDefaultBGM(string bgm, float volume = 80, float pitch = 100)
		{
			if (!string.IsNullOrEmpty(bgm))
			{
				...
				setDefaultBGM(Game.FileTest.initialize(bgm, volume, pitch) as IAudioBGM, volume, pitch);
			}
			//if (bgm != null && bgm.name != "")
			//{
			//	@defaultBGM = null;
		...
		public void me_play(string me)
		{
			if (!string.IsNullOrEmpty(me))
			{
				...
				me_play(Game.FileTest.initialize(me, 80, 100) as IAudioME);
			}
			//if (me != null && me.name != "")
			//{
			...
			//		vol = vol.to_i;
			//		AudioManager.AudioHandler.me_play("Audio/ME/" + me.name, vol, me.pitch);
			//	}
			//}
			//else
			//{
		...
		public void me_play(IAudioME me)
		{
			...
			if (me != null && me.name != "")
			{
				if (Game.FileTest.audio_exist("Audio/ME/" + me.name))
				{
					float vol = me.volume;
					if (Game.GameData.PokemonSystem != null) vol *= (Game.GameData.PokemonSystem.bgmvolume / 100.0f);
					vol = (int)vol;
					//AudioManager.AudioHandler.me_play("Audio/ME/" + me.name, vol, me.pitch);
					if (Game.GameData.Audio is IAudio audio) audio.me_play("Audio/ME/" + me.name, vol, me.pitch);
				}
			}
			else
			{
				//AudioManager.AudioHandler.me_stop();
				if (Game.GameData.Audio is IAudio audio) audio.me_stop();
			}
			//(Game.GameData as Game).Graphics.frame_reset();
		}
		...
		public void me_fade(float time)
		{
			...
			if (defaultBGM == null && Game.GameData.Audio is IAudio audio) audio.me_fade((int)Math.Floor(time * 1000));
		}
		...
		public void me_stop()
		{
			...
			if (defaultBGM == null && Game.GameData.Audio is IAudio audio) audio.me_stop();
		}
		...
		public void bgs_play(IAudioBGS bgs)
		{
			...
			{
				if (Game.FileTest.audio_exist("Audio/BGS/" + bgs.name))
				{
					float vol = bgs.volume;
					if (Game.GameData.PokemonSystem != null) vol *= (Game.GameData.PokemonSystem.sevolume / 100.0f);
					vol = (int)vol;
					//AudioManager.AudioHandler.bgs_play("Audio/BGS/" + bgs.name, vol, bgs.pitch);
					if (Game.GameData.Audio is IAudio audio) audio.bgs_play(bgs.name, vol, bgs.pitch);
				}
			}
			else
			{
				...
				if (Game.GameData.Audio is IAudio audio) audio.bgs_stop();
			}
			(Game.GameData as Game).Graphics.frame_reset();
		}
		/// <inheritdoc/>
		...
		public void bgs_fade(float time)
		{
			@bgs_position = 0;
			@playing_bgs = null;
			//AudioManager.AudioHandler.bgs_fade((int)Math.Floor(time * 1000));
			if (Game.GameData.Audio is IAudio audio) audio.bgs_fade((int)Math.Floor(time * 1000));
		}
		...
		public void se_play(string se)
		{
			if (!string.IsNullOrEmpty(se))
			{
				...
				se_play(Game.FileTest.initialize(se, 80, 100) as IAudioSE);
			}
			//if (se != null && se.name != "")
			//{
			...
			//		vol = vol.to_i;
			//		AudioManager.AudioHandler.se_play("Audio/SE/" + se.name, vol, se.pitch);
			//	}
			//}
		}
		...
		public void se_play(IAudioSE se)
		{
			...
			if (se != null && se.name != "")
			{
				if (Game.FileTest.audio_exist("Audio/SE/" + se.name))
				{
					float vol = se.volume;
					if (Game.GameData.PokemonSystem != null) vol *= (Game.GameData.PokemonSystem.sevolume / 100.0f);
					vol = (int)vol;
					//AudioManager.AudioHandler.se_play("Audio/SE/" + se.name, vol, se.pitch);
					//if (Game.GameData.Audio is IAudio audio) audio.se_play(se.name, vol, se.pitch);
					se_play(se.name, vol, se.pitch);
				}
			}
		}
		...
		{
			if (Game.GameData is IAudio audio) audio.se_play(filename, volume, pitch);
		}

		public void se_stop()
		{
			if (Game.GameData.Audio is IAudio audio) audio.se_stop();
		}
		...
		{
			if (Game.GameData is IAudio audio) audio.bgm_play(filename, volume, pitch, position);
		}

		void IAudio.bgs_play(string filename, float volume, float pitch)
		{
			if (Game.GameData is IAudio audio) audio.bgs_play(filename, volume, pitch);
		}

		void IAudio.me_play(string filename, float volume, float pitch)
		{
			if (Game.GameData is IAudio audio) audio.me_play(filename, volume, pitch);
		}
	}
}