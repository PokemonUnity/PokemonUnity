...
#pragma warning disable 0162 //Warning CS0162  Unreachable code detected
		...
		public int SecondPartyBegin(int battlerIndex) {
			if (IsOpposing(battlerIndex)) {
				...
				return @fullparty2 ? (Game.GameData.Global?.Features.LimitPokemonPartySize ?? Core.MAXPARTYSIZE) : (int)(Core.MAXPARTYSIZE * .5);
			}
			else {
				//return @fullparty1 ? 6 : 3; //split in half for doubles
				return @fullparty1 ? (Game.GameData.Global?.Features.LimitPokemonPartySize ?? Core.MAXPARTYSIZE) : (int)(Core.MAXPARTYSIZE * .5);
			}
		}

		public int PartyLength(int battlerIndex) {
			if (IsOpposing(battlerIndex)) {
				return (@opponent.Length > 0) ? SecondPartyBegin(battlerIndex) : (Game.GameData.Global?.Features.LimitPokemonPartySize ?? Core.MAXPARTYSIZE);
			}
			else {
				return @player.Length > 0 ? SecondPartyBegin(battlerIndex) : (Game.GameData.Global?.Features.LimitPokemonPartySize ?? Core.MAXPARTYSIZE);
			}
		}
		...
		public bool CanMegaEvolve (int index) {
			...
			if (debug && Input.press((int)PokemonUnity.Interface.InputKeys.DEBUG)) return true;
		...
		public void MegaEvolve(int index) {
			...
			Core.Logger.Log($"[Mega Evolution] #{_battlers[index].ToString()} Mega Evolved");
			int side=IsOpposing(index) ? 1 : 0;
			int owner=GetOwnerIndex(index);
		...
		public void PrimalReversion(int index) {
			...
			Core.Logger.Log($"[Primal Reversion] #{_battlers[index].ToString()} Primal Reverted");
		}
		#endregion
		...
		public void Call(int index) {
			...
			Core.Logger.Log($"[Call to Pokémon] #{owner.name} called to #{_battlers[index].ToString(true)}");
		...
		public void Judge() {
			...
			Core.Logger.Log("[Remaining Pokemons => Player 1: #{0} vs. Player 2: #{1}]", PokemonCount(@party1), PokemonCount(@party2));
		...
		public virtual void CommandPhase() {
			...
			for (int i = 0; i < battlers.Length; i++) {   // Reset choices if commands can be shown
				...
				if (CanShowCommands(i) || _battlers[i].isFainted()) {
					...
					if (@doublebattle && !IsDoubleBattler(i)) {
						Core.Logger.Log($"[Reusing commands] #{_battlers[i].ToString(true)}");
					}
				}
			}
