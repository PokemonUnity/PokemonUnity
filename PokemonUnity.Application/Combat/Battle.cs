...
namespace PokemonUnity.Combat
{
	...
	public partial class Battle : IBattle, IHasDisplayMessage
	{
		...
		/// Player trainer
		/// </summary>
		public ITrainer[] player { get; private set; }
		/// <summary>
		/// Opponent trainer; if null => wild encounter
		/// </summary>
		public ITrainer[] opponent { get; private set; }
		/// <summary>
		/// Player's Pokémon party
		/// </summary>
		public IPokemon[] party1 { get; private set; }
		/// <summary>
		/// Foe's Pokémon party
		/// </summary>
		public IPokemon[] party2 { get; private set; }
		/// <summary>
		...
		/// </summary>
		public IList<int> party1order { get; private set; }
		/// <summary>
		/// Order of Pokémon in the opponent's party
		/// </summary>
		public IList<int> party2order { get; private set; }
		/// <summary>
		...
		/// </summary>
		public IBattler[] battlers { get; private set; }
		/// <summary>
		...
		public IBattler[] priority { get; private set; }
		...
		public virtual void pbStorePokemon(IPokemon pokemon)
		{
			...
			string creator = Game.GameData.Player.IsCreator ? Game.GameData.Player.Name : Game.GameData.Player.GetStorageCreator();
		...
		public string pbGetMegaRingName(int battlerIndex) {
			if (pbBelongsToPlayer(battlerIndex)) {
				foreach (Items i in Core.MEGARINGS) {
					...
					if (Game.GameData.Bag.GetItemAmount(i)>0) return i.ToString(TextScripts.Name);
				}
			}
		...
		public bool pbHasMegaRing(int battlerIndex) {
			...
			foreach (Items i in Core.MEGARINGS) {
				...
				if (Game.GameData.Bag.GetItemAmount(i)>0) return true;
			}
			return false;
		}
		#endregion
		...
		public bool CanChooseMove(int idxPokemon, int idxMove, bool showMessages, bool sleeptalk = false)
		{
			...
			if (opp1.IsNotNullOrNone() && opp1.effects.Imprison)
			{
				...
				{
					if (showMessages) pbDisplayPaused(Game._INTL("{1} can't use the sealed {2}!", thispkmn.ToString(), thismove.id.ToString(TextScripts.Name)));
					GameDebug.Log($"[CanChoose][#{opp1.ToString()} has: #{opp1.moves[0].id.ToString(TextScripts.Name)}, #{opp1.moves[1].id.ToString(TextScripts.Name)}, #{opp1.moves[2].id.ToString(TextScripts.Name)}, #{opp1.moves[3].id.ToString(TextScripts.Name)}]");
					return false;
				}
			}
			if (opp2.IsNotNullOrNone() && opp2.effects.Imprison)
			{
				...
				{
					if (showMessages) pbDisplayPaused(Game._INTL("{1} can't use the sealed {2}!", thispkmn.ToString(), thismove.id.ToString(TextScripts.Name)));
					GameDebug.Log($"[CanChoose][#{opp2.ToString()} has: #{opp2.moves[0].id.ToString(TextScripts.Name)}, #{opp2.moves[1].id.ToString(TextScripts.Name)}, #{opp2.moves[2].id.ToString(TextScripts.Name)}, #{opp2.moves[3].id.ToString(TextScripts.Name)}]");
					return false;
				}
			}
			if (thispkmn.effects.Taunt > 0 && thismove.Power == 0) {//.BaseDamage
				if (showMessages) pbDisplayPaused(Game._INTL("{1} can't use {2} after the taunt!", thispkmn.ToString(), thismove.id.ToString(TextScripts.Name)));
				return false;
			}
			...
			if (thismove.id==thispkmn.effects.DisableMove && !sleeptalk) {
				if (showMessages) pbDisplayPaused(Game._INTL("{1}'s {2} is disabled!", thispkmn.ToString(), thismove.id.ToString(TextScripts.Name)));
				return false;
			}
		...
		public virtual void pbAutoChooseMove(int idxPokemon, bool showMessages=true) {
			...
			if (thispkmn.effects.Encore>0 &&
				...
				GameDebug.Log($"[Auto choosing Encore move] #{thispkmn.moves[thispkmn.effects.EncoreIndex].id.ToString(TextScripts.Name)}");
		...
		bool IBattle.pbUseItemOnPokemon(Items item,int pkmnIndex,IBattler userPkmn,IHasDisplayMessage scene) {
			...
			if (!ret && pbBelongsToPlayer(userPkmn.Index)) {
				if (Game.GameData.Bag.CanStore(item)) {
					Game.GameData.Bag.AddItem(item);
				}
				else {
					...
					if (!@battlers[i].IsOpposing(idxPokemon) &&
						...
						if (pbBelongsToPlayer(@battlers[i].Index)) {
							if (Game.GameData.Bag.CanStore(idxItem)) {
								Game.GameData.Bag.AddItem(idxItem);
							}
							else {
								//throw new Exception(Game._INTL("Couldn't return unused item to Bag somehow."));
								GameDebug.LogError(Game._INTL("Couldn't return unused item to Bag somehow."));
		...
		public virtual int pbRun(int idxPokemon,bool duringBattle=false) {
			...
			int speedPlayer=@battlers[idxPokemon].pokemon.speed;
			...
			if (!opposing.isFainted()) {
				int speedEnemy=opposing.pokemon.speed;
		...
		public virtual bool pbOnActiveOne(IPokemon pkmn,bool onlyabilities=false,bool moldbreaker=false) {
		...
		public void pbStartBattleCore(bool canlose) {
			...
			#region Initialize opponents in double battles;
				...
				if (@opponent.Length > 0) {
					...
					@battlers[1]	(@party2[sendout1],(sbyte)sendout1,false);
						...
						do { //loop
							...
							if (cmd==MenuCommands.FIGHT) { // Fight
								...
								if (@choices[0].Action==ChoiceAction.UseItem && Game.GameData.Bag != null && Game.GameData.Bag.CanStore((Items)@choices[0].Index)) {
									Game.GameData.Bag.AddItem((Items)@choices[0].Index); //@choices[0][1]
								}
								pbCommandPhase();
								return;
							}
		...
		public BattleResults pbEndOfBattle(bool canlose=false) {
			...
			foreach (IPokemon i in Game.GameData.Player.Party) {
		...
		bool IBattle.pbIsOpposing(int index)
		{
			throw new NotImplementedException();
		}
